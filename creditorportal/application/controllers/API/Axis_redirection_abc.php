...
class Axis_redirection_abc extends MY_AbcSessionCheck
{
    ...
    function __construct()
    {
        ...
        $this->product_id = $aD2CSession['product_id'];
    ...
    public function show_session_timeoutpage()
    {
        ...
        $this
            ->load
            ->view('session_timeout_view_generic');
    }
    ...
    public function payment_redirection()
    {
        ...
        $this->Abc_m->insertOrUpdateUserActivity('6');

        if ($emp_id)
        {
            $query = $this
			->db
			->query("SELECT ed.customer_name,ed.lead_id,ed.emp_id,epd.policy_detail_id,ed.email,ed.mob_no,ed.product_id,sum(p.premium) as premium,mpst.payment_url,p.status,p.id as proposal_id,GROUP_CONCAT(epd.combine_coi) as combine_coi FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p,payment_details as pd WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.product_name AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND p.id = pd.proposal_id AND ed.emp_id= '".$emp_id."' GROUP BY p.emp_id")->row_array();

            // Combine COI logic starts
            $combineCoiFlagArr = explode(',',$query['combine_coi']);
            $temp = array_filter($combineCoiFlagArr, function($value){
                return $value > 0;
            });
            $checkCombineCoi = count($temp);
			...
			if(!empty($query))
			{
                ...
                $paymentConfig = $this->db->query("SELECT * FROM main_product_master as mpm where mpm.main_product_id = '".$productId."'")->row_array();
				
                if ($query['status'] != 'Payment Pending')
                {
                    redirect(base_url("success_view/" . $emp_id));
                }
                else
                {
                    ...
                    $lead_data = $this
                        ->obj_api_new
                        ->get_all_quote_call($emp_id,$checkCombineCoi);
                    if ($lead_data['status'] == 'Success')
                    {
                        ...
                        $Source = $paymentConfig["payment_source"];
                        $Vertical = $paymentConfig["payment_vertical"];
                        $PaymentMode = $paymentConfig["payment_mode"];


                        $ReturnURL = base_url("success_view/" . $emp_id);
                    ...
                    else
                    {
                        ...
                        redirect(base_url("/error_view/" . $emp_id . "/1"));
    ...
    public function payment_error_view($emp_id, $status)
    {
        ...
        //$emp_id = $this->emp_id;
        $title_head = 'Error Page';
        $lead_arr = $this
            ->db
            ->query("select lead_id,email,product_id from employee_details where emp_id = '$emp_id' ")->row_array();
        $lead_id = $lead_arr['lead_id'];
        $email = $lead_arr['email'];

        $this
            ->load
            ->abc_portal_template('payment_error_view', compact('emp_id', 'lead_id', 'email', 'status','title_head'),true,$lead_arr['product_id']);
    ...
    public function payment_success_view($emp_id_encrypt)
    {
        ...
        $query = $this
            ->db
            ->query("SELECT GROUP_CONCAT(p.id) proposal_id,ed.emp_id,ed.product_id,ed.lead_id,mpst.product_code FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p,payment_details AS pd WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.product_name AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND p.id = pd.proposal_id AND ed.emp_id='" . $emp_id . "' GROUP BY p.emp_id")->row_array();
        ...
        if ($query)
        {
            if (isset($TxRefNo))
            {
                $request_arr = ["lead_id" => $query['lead_id'], "req" => $encrypted, "res" => $decrypted, "product_id" => $query['product_id'], "type" => "payment_response_post"];
                //print_pre($TxMsg);exit;
                $dataArray['tablename'] = 'logs_docs';
                $dataArray['data'] = $request_arr;
                $this
                    ->Logs_m
            ...
            if (isset($Registrationmode))
            {
                ...
                if ($query_emandate > 0)
                {
                    ...
                    $request_arr = ["lead_id" => $query['lead_id'], "req" => json_encode($dataArray), "res" => "" , "product_id"=>$query['product_id'], "type" => "insert_emandate_data"];
                    $dataArray['tablename'] = 'logs_docs';
                    $dataArray['data'] = $request_arr;
                    $this
                        ->Logs_m
                ...
                else
                {
                    ...
                    $arr = ["lead_id" => $query['lead_id'], "product_id"=>$query['product_id'],"TRN" => $EMandateRefno, "status_desc" => $EMandateStatusDesc, "status" => $mandate_status, "mandate_date" => date('Y-m-d h:i:s', strtotime($EMandateDate)) , "Registrationmode" => $Registrationmode, "EMandateFailureReason" => $EMandateFailureReason, "MandateLink" => $MandateLink];
            ...
            if ($payment_data['TxStatus'] == 'success')
            {
                ...
                {
                    //HB emandate call
                    if(isset($EMandateStatus) && $EMandateStatus == 'MS'){
                        $this->obj_api_new->emandate_HB_call($query['emp_id']);
                    }
                    ...
                    $dataTable = $this
                        ->db
                        ->query("SELECT apr.certificate_number,GROUP_CONCAT(epd.product_name) AS product_name,apr.relationship_code,apr.fr_ids,GROUP_CONCAT(p.proposal_no) AS proposal_no
                            FROM api_proposal_response AS apr
                            LEFT JOIN proposal AS p ON apr.proposal_no_lead = p.proposal_no
                            LEFT JOIN employee_policy_detail AS epd ON p.policy_detail_id = epd.policy_detail_id
                            LEFT JOIN employee_details AS e ON e.emp_id = p.emp_id
                            WHERE e.emp_id = $emp_id GROUP BY apr.certificate_number")->result_array();
                    ...
                    if ($data_policy)
                    {
                        ...
                        if ($activity_stage["type"] <= 7)
                        {
                            ...
                            // $this
                            //     ->db
                            //     ->update("user_activity_abc", ['type' => 7]);
                            $this->Abc_m->insertOrUpdateUserActivity(7,$emp_id);
                        }
                        ...
                        /* ABC Policy Issued CRM Call */
                        if($query['product_id'] == 'ABC'){
                            $this->obj_api_new->crmLeadUpdatePolicyIssuedAbc($emp_id);
                        }
                        /* END */

                        $this
                            ->load
                            ->abc_portal_template("thankyou_view_abc", compact('data_policy', 'data', 'extra_check', 'dataTable','MandateLink_data','title_head'),true,$query['product_id']);
                    }
                ...
                else
                {
                    ...
                    //echo "in";exit;
                    redirect(base_url("/error_view/" . $emp_id . "/0"));
            ...
            else
            {
                ...
                if ($activity_stage["type"] <= 7)
                {
                    ...
                    //$this->Abc_m->insertOrUpdateUserActivity(7);
                }
                $dataTable = $this
                    ->db
                    ->query("SELECT epd.product_name,apr.certificate_number,apr.relationship_code,apr.fr_ids,p.proposal_no FROM api_proposal_response AS apr
                ...
                if (in_array('Success', $arr))
                {
                    ...
                    $this
                        ->load
                        ->abc_portal_template("thankyou_view_abc", compact('data_policy', 'data', 'extra_check', 'dataTable','title_head'));
                }
                else if (!in_array('Success', $arr) && $data['status'] == 'Payment Received')
                {
                    ...
                    $this
                        ->load
                        ->abc_portal_template("thankyou_view_abc", compact('data', 'extra_check', 'dataTable','title_head'));
                }
                else
                {
                    $extra_check = 3;
    ...
    function logs_docs_insert_generic($lead_id,$request, $type,$product_id,$response = "")
    {
        $request_arr = ["lead_id" => $lead_id, "req" => json_encode($request) , "type" => $type, "product_id" => $product_id];
        // $this->db->insert("logs_docs", $request_arr);
        $logs_array['data'] = $request_arr;
        $this
            ->Logs_m
    ...
    function dropoff_journey($emp_id_encrypt, $send_type)
    {
        ...
        $val = $this
            ->db
            ->query("select ed.json_qote,ed.mob_no,ed.email,ed.lead_id,ed.product_id,ua.type,ed.emp_id from employee_details as ed,user_activity_abc as ua where ed.emp_id = ua.emp_id AND ua.status = 1 AND ua.emp_id = '$emp_id'")->row_array();
        /*echo $this->db->last_query();
         print_pre($val);exit;*/

        //Add mob_no/product_id in session for return journey 10-03-2021
        ...
        if ($val['type'] == 1)
        {
            ...
            $this->logs_docs_insert_generic($lead_id, $arr, $logs_type, $val['product_id']);
            redirect(base_url('comprehensive_products' . $utm_string));
        }
        // Add encryption for policy_id 10-03-2021
        elseif ($val['type'] == 2)
        {
            $arr['link'] = base_url('generate_quote/' . encrypt_decrypt_password($policy_id));
            // $this->logs_docs_insert($lead_id, $arr, $logs_type);
            $this->logs_docs_insert_generic($lead_id, $arr, $logs_type, $val['product_id']);
            redirect(base_url('generate_quote/' . encrypt_decrypt_password($policy_id) . $utm1_string));
        }
        elseif ($val['type'] == 3)
        {
        ...
        elseif ($val['type'] == 4)
        {
            ...
            // $this->logs_docs_insert($lead_id, $arr, $logs_type);
            $this->logs_docs_insert_generic($lead_id, $arr, $logs_type, $val['product_id']);
            redirect(base_url('member_review' . $utm_string));
        }
        // Change URL to payment_redirection_abc 10-03-2021
        elseif ($val['type'] == 5 || $val['type'] == 6)
        {
            $arr['link'] = base_url('payment_redirection_abc');
            // $this->logs_docs_insert($lead_id, $arr, $logs_type);
            $this->logs_docs_insert_generic($lead_id, $arr, $logs_type, $val['product_id']);
            redirect(base_url('payment_redirection_abc' . $utm_string));
        }
        else
        {
            ...
            // $this->logs_docs_insert($lead_id, $arr, $logs_type);
            $this->logs_docs_insert_generic($lead_id, $arr, $logs_type, $val['product_id']);
            redirect(base_url('success_view/' . $emp_id . $utm_string));
        }
    }
