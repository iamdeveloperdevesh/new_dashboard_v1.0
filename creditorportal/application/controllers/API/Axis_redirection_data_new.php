            ...
            if ($checksum != $hashData) {
                ...
                if (empty($aJSONData)) {
                    ...
                    if ($isValid) {
                        ...
                        /* Check whether lead id already exists for any other product in same DB */
                        $existingLeadOtherProduct = $this->db->select("lead_id, product_id")
                            ->from("employee_details ed")
                            ->where("ed.lead_id", $this->aFields['crm_lead_id'])
                            ->where_not_in("product_id",['D2C2','D01','D02'])
                            ->limit(1)
                            ->get()
                            ->result_array();
                        if(!empty($existingLeadOtherProduct)){
                            $message = "Policy already purchased with this lead id.";
                            $this->load->retail_template('Retail/redirection_msgs', compact('message'));
                        } else {
                            ...
                            $resultCombineCheck = $this->db->select("lead_id")
                                ...
                                ->where("ed.lead_status != ","Cancelled")
                            ...
                            if (!empty($resultCombineCheck)) {
                                ...
                                if (strcmp($this->aFields['crm_lead_id'], $existingLeadID) !== 0) {
                                    ...
                                    else{//Reverse feed - QE - sonal add else
                                        $this->session->set_flashdata('flashdata_lead_continue',$data);
                                        redirect('/continue_lead_data');
                                    }
                        ...
                        // print_pre(!empty($existingLeadOtherProduct));exit;
                    }
                }
            }
        }
...
//Reverse feed - QE - sonal
    ...
    function is_valid_data() {
        ...
        // Valid Mobile Number
        $this->aFields['mobile_number'] = substr($this->aFields['mobile_number'], -10); // Taking last 10 digits of mobile number
        ...
        if(empty($this->aFields['lg_sol_id'])){
            $otherErrMsg = "<br/> Blank Sol ID received.";
        }
    ...
    function continue_lead_data() {
        if (!empty($this->input->post('cust_data_json')) && !empty($this->input->post('create_new'))) {
            ...
            if ($isCreateNew == 'yes') {
                // print_pre('Want to create new');exit;
                ...
                */
                ...
                if ($rsEmp->num_rows() > 0) {
                    ...
                    else{//Reverse feed - QE - sonal - added else
                    ...
                    if(!empty($detail['existingLeads'])){
                        foreach ($detail['existingLeads'] as $key => $lead) {
                            $empId = $lead['emp_id'];
                            if(empty($lead['proposalsCreated'])){
                                // Mark Lead as 'Cancelled'
                                $this->db->where('emp_id',$empId);
                                $leadStatusUpdate = $this->db->update('employee_details',['lead_status'=>'Cancelled']);
                            } else {
                                $statusBox = [];
                                foreach ($lead['proposalsCreated'] as $key => $proposal) {
                                    $statusBox[$proposal['id']] = $proposal['status'];
                                }
                                if(in_array('Payment Received',$statusBox) || in_array('Success',$statusBox)){
                                    continue;
                                } else {
                                    $proposalIdArr = array_keys($statusBox);
                                    // Mark Lead & proposal as 'Cancelled'
                                    $this->db->where_in('id',$proposalIdArr);
                                    $proposalStatusUpdate = $this->db->update('proposal',['status'=>'Cancelled']);
                                    $this->db->where('emp_id',$empId);
                                    $leadStatusUpdate = $this->db->update('employee_details',['lead_status'=>'Cancelled']);
                                }
                            }
        ...
        } elseif(!empty($this->session->flashdata('flashdata_lead_continue'))) {
            //!empty($this->session->flashdata('flashdata_lead_continue'))){
            ...
            /*
              ...
              i.e. Proposal does not exist!!
             */
            $rsEmp = $this->db->select("emp_id, product_id, lead_status")->where(["lead_id" => $lead_id,"product_name"=>$aCustJSON['product_name']])->get("employee_details");
            $existingLeadRecord = $rsEmp->row_array();
            // print_pre($flashData);exit;
            if ($rsEmp->num_rows() <= 0) {
                $message = "No data found";
                $this->load->retail_template('Retail/redirection_msgs', compact('message'));
            } else if ($existingLeadRecord['lead_status'] == 'Cancelled') {
                $message = "This lead is cancelled, cannot proceed with this lead number.";
                $this->load->retail_template('Retail/redirection_msgs', compact('message'));
            } else {
                //insert log
                $aRow = $rsEmp->row();
    ...
    function dropoff_journey($emp_id_encrypt,$send_type) {
        ...
        $val = $this->db->query("select ed.json_qote,ed.product_id,ed.mob_no,ed.email,ed.lead_id,ua.type,ed.emp_id,ed.lead_status from employee_details as ed,user_activity as ua where ed.emp_id = ua.emp_id AND ua.status = 1 AND ua.emp_id = '$emp_id'")->row_array();

        if($val['lead_status'] == 'Cancelled'){
            $message = "This lead is cancelled, cannot proceed with this lead number.";
            $this->load->retail_template('Retail/redirection_msgs', compact('message'));
        } else {
    ...
    public function checkExistingLeadOtherProduct(){
