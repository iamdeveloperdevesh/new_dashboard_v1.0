...
class Axis_new_retail extends CI_controller
{
    ...
    function __construct()
    {
		...
		$this->load->model("Retail/Dashboard_m", "Dashboard_m", true);
	...
	public function check_error_data() {
		...
		if($query_check > 0){
			...
			if($query['payment_status'] == 'No Error' && $query['proposal_status'] == 'Cancelled'){
				$data = array(
					"status" => "1",
					"check" => "5",
	...
	public function payment_redirect_view($emp_id_encrypt)
	{
		...
		$this->Dashboard_m->insertOrUpdateUserActivity('4');
		if($emp_id){
//Revese feed change status - QE
		$quote_exp_check = common_quote_expired($emp_id);
			...
			$query = $this->db->query("SELECT ed.customer_name,mpst.product_name,ed.lead_id,ed.emp_id,ed.product_id,epd.policy_detail_id,mpst.payment_source_code,ed.email,ed.mob_no,sum(p.premium) as premium,mpst.payment_url,p.status,mpst.product_code,pd.bank_name,ed.acc_no,ed.lead_status FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p,payment_details as pd WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.id AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND p.id = pd.proposal_id AND ed.emp_id=".$emp_id)->row_array();
			if(!empty($query))
            {
                if($query['product_id'] == 'D01')
                {
                    $productname = 'Group Activ Health and Group Activ Secure';
                }
                elseif($query['product_id']  == 'D02')
            	{//echo 123;die;
			...
			if(!empty($query))
			{
				if($query['status'] == 'Cancelled'){
					echo "Cannot initaiate payment for cancelled lead";
				} else {
					if($query['status'] != 'Payment Pending'){
						redirect(base_url('retail/payment_return_view/'.$emp_id_encrypt));
					// print_pre("Return to payment return view Proposal status is not Pending");
					}else{
						...
						if($lead_data['status'] == 'Success'){
							...
							if($result && $result['Status']){
								...
								// Insert log for payment gateway redirection START
								$logs_array['data'] = ["type" => "redirected_to_pg","req" => json_encode($dataPost), "lead_id" => $query['lead_id'],"product_id" => $query['product_id']];
								$this->Logs_m->insertLogs($logs_array);
								// Insert log for payment gateway redirection END
	...
	public function payment_return_view_new($emp_id_encrypt)
	{
		...
		// Insert log for return from payment gateway START
		$logs_array['data'] = ["type" => "returned_from_pg","res" => $this->input->post('RESPONSE'), "lead_id" => $query['lead_id'],"product_id" => $query['product_id']];
		$this->Logs_m->insertLogs($logs_array);
		// Insert log for return from payment gateway  END
		...
		if($query){
			...
			if($payment_data['TxStatus'] == 'success'){
				...
				if($check_result['Status'] == 'Success'){
					...
					$data_policy = $this->db->query("SELECT apr.*,p.policy_detail_id,epd.policy_no, SUM(apr.gross_premium) AS sum_response_premium, p.premium AS sum_premium, GROUP_CONCAT(apr.certificate_number) AS all_coi_number, GROUP_CONCAT(fr_ids) AS all_fr_ids FROM api_proposal_response AS apr, proposal AS p,employee_policy_detail AS epd WHERE apr.emp_id = '$emp_id' AND p.id = apr.proposal_id AND epd.policy_detail_id = p.policy_detail_id GROUP BY apr.proposal_no_lead")->result_array();
					...
					$this->Dashboard_m->insertOrUpdateUserActivity('5');
