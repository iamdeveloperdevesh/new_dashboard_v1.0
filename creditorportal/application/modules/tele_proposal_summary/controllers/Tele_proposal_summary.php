...
class Tele_proposal_summary extends CI_Controller {
...
// echo $start_date;
            if($date_diff < 5 && $query['status'] != 'Rejected' && $link_expire_check)
            {
                ...
                if($product_id == 'T01'){
                    ...
                    $parent_ids = $this->db->select('p.policy_parent_id,tlm.id')
                        ->from('employee_details l')
                        ->join('product_master_with_subtype p', 'l.product_id = p.product_code')
                        ->join('tls_agent_mst tlm', 'tlm.id = l.av_code')
                        ->where('l.emp_id', $emp_id_decrypt)
                        ->get()
                        ->row_array();
                    ...
                    if($is_makerchecker_journey['is_makerchecker_journey']=='yes'){
                        ...
                        $parent_ids=$this->db->query("SELECT l.is_makerchecker_journey,p.policy_parent_id, tlm.base_id FROM employee_details l JOIN product_master_with_subtype p ON l.product_id = p.product_code JOIN tls_base_agent_tbl tlm ON l.assigned_to = tlm.base_id WHERE  l.emp_id = '".$emp_id_decrypt."'")->row_array();
                    }

                    $parent_id = $parent_ids['policy_parent_id'];
                    $agent_id = encrypt_decrypt_password($parent_ids['id'],'E');
                 //   print_r($parent_ids);die;
                }
                $telSalesSession['telesales_session'] = ['emp_id' => $emp_id_decrypt,'parent_id' => $parent_id,'agent_id'=> ($agent_id),'is_maker_checker'=>$is_makerchecker_journey['is_makerchecker_journey']];
                $this->session->set_userdata($telSalesSession);
                $telSalesSession = $this->session->userdata('telesales_session');
               // print_pre($telSalesSession);exit;
                $this->customer_summary();
            //    redirect(base_url('customer_summary'));
    ...
    {//echo 123;die;
        ...
        $this->load->model("teleproposal/Teleproposal_m", "obj_home", true);
        $data = $this->obj_home->get_common_data();
        $data['agent_details'] = $this->obj_home->get_agent_details($this->agent_id);
        $data['employee_declaration'] = $this->obj_home->health_declaration_emp_data($this->parent_id);
//print_r($data);echo $this->parent_id;exit;
