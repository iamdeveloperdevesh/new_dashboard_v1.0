...
class Create_proposal extends MY_TelesalesSessionCheck
{
	...
	public function update_password_all(){
		...
		//first update -  ALTER TABLE tls_agent_mst MODIFY `password` VARCHAR(255);
		...
		foreach($tls_agent_mst as $single_agent){
			
			$concat_pw = "@!ABHI2022";
			$new_password = $single_agent['agent_id'].$concat_pw;
			
			$encrypted_password = encrypt_decrypt_password($new_password,"E");
			
			$update_net_premium = $this->db->set('password', $encrypted_password);
			$this->db->where('id',$single_agent['id']);
			$this->db->update('tls_agent_mst');
		}



		$tls_agent_mst_outbound = $this
		->db
		->select("*")
		...
		foreach($tls_agent_mst_outbound as $single_agent){
			...
			$update_net_premium = $this->db->set('password', $encrypted_password);
			$this->db->where('id',$single_agent['id']);
			$this->db->update('tls_agent_mst_outbound');
		}
		...
		foreach($tls_master_do as $single_agent){
			
			$concat_pw = "@!ABHI2022";
			$new_password = $single_agent['do_id'].$concat_pw;
			
			$encrypted_password = encrypt_decrypt_password($new_password,"E");
		
			$update_net_premium = $this->db->set('password', $encrypted_password);
			$this->db->where('id',$single_agent['id']);
			$this->db->update('tls_master_do');
		}
		...
		foreach($employee_details as $single_agent){
			
			$concat_pw = "@!ABHI2022";
			$new_password = $concat_pw;
			
			$encrypted_password = encrypt_decrypt_password($new_password,"E");
		
			$update_net_premium = $this->db->set('password', $encrypted_password);
			$this->db->where('emp_id',$single_agent['emp_id']);
	...
	$emp_id = $this->emp_id;
		...
		$data['disposition'] = $this->db->select('*')->from('disposition_master')->where('display', 1)->where('id NOT IN (55,56)')->get()->result_array();


		// $lead_creation  = $this->db->select('employee_details.created_at,employee_details.axis_process,employee_details.makerchecker,employee_details.is_makerchecker_journey,tls_agent_mst.agent_name')->from('employee_details,tls_agent_mst')
		// ->where('employee_details.assigned_to = tls_agent_mst.id')
		// ->where('emp_id',$emp_id)
		// ->get()->row_array();

			//upendra - maker/checker - 30-07-2021
			$check_maker_checker = $this->db->select("makerchecker,is_makerchecker_journey")
			->from('employee_details')
			->where('emp_id',$emp_id)
			->get()->row_array();

			// print_r($check_maker_checker);exit;
			...
			$lead_creation  = $this->db->select('employee_details.makerchecker,employee_details.emp_id,employee_details.is_makerchecker_journey,employee_details.created_at,employee_details.axis_process,tls_base_agent_tbl.base_agent_name as agent_name')->from('employee_details,tls_base_agent_tbl')
			...
			$lead_creation  = $this->db->select('employee_details.makerchecker,employee_details.is_makerchecker_journey,employee_details.created_at,employee_details.axis_process,tls_agent_mst.agent_name')->from('employee_details,tls_agent_mst')
			->where('employee_details.assigned_to = tls_agent_mst.id')
			->where('emp_id',$emp_id)
			->get()->row_array();

			$lead_creation_merge[0]['agent_name'] ='AV';		
		...
		if($lead_creation['makerchecker']=='checker'&&$lead_creation['is_makerchecker_journey']=='yes'){
			...
			$data['checker_sendbackto_do_agent_name']='';
		}		
	...
	public function master_axis_vendor()
	{
		...
		if($location_post != null || !empty($location_post)){
			...
			$query = $this->db->query("select DISTINCT axis_vendor,axis_vendor_id from tls_axis_vendor");	
		}
	...
	//	print_pre($policy_declaration_data);exit;
		...
		{   
			...
			$data .= '<tr>
							...
							<td style="border:none;"><input type="checkbox"  name="' . $value['p_declare_id'] . '" id="' . $value['p_declare_id'] . '" class="" value="Yes"><input type="checkbox"  name="' . $value['p_declare_id'] . '" id="' . $value['p_declare_id'] . '" class="" value="No" checked style="display:none"></td>
...
$lead_creation  = $this->db->select('employee_details.created_at,tls_base_agent_tbl.base_agent_name as agent_name')->from('employee_details,tls_base_
agent_tbl')
                ->where('employee_details.assigned_to = tls_base_agent_tbl.base_id')
                        ->where('emp_id',$emp_id)
                                ->get()->row_array();
	...
	public function summary()
	{
		...
		// print_r($data['policy_details']);exit;
 ...
 /*dedupe logic changes function to generate unique reference no*/
	...
	public function update_tele_net_premium(){

		

		// $premium_array = ["12199","17299","21299","24699","16200","19699","12800","18200","22399","25999","17000","20600","13299","19000","23400","27199","17699","21400","16899","24200","28800","32799","21500","26398","18200","25999","30999","35399","23199","28199","19499","27899","33200","37900","24799","30000","2001","4001","3000","6001","4001","8001","5300","7499","8900","10199","6699","7399","2799","3700","4100","4400","3199","3499","4800","6699","7899","8999","6000","6599","4199","5799","6800","7700","5200","5700","11500","16399","18700","20899","13799","15999","6000","8400","9400","10300","7000","7899","10199","14499","16499","18399","12199","14199","8899","12700","14300","15800","10499","12399","8000","11500","14500","17000","11000","14000","10500","14500","18500","21500","14000","18000","11000","16000","19500","23500","15400","18800","11500","16500","20500","23900","15900","19000","11900","17000","21000","24400","16000","19500","8500","12000","15000","17500","11500","14500","11000","15000","19000","22000","14500","18500","13000","17500","22000","26500","17000","21500","15000","20500","26000","30500","20000","25500","16500","23000","28600","32600","21500","26200","300","600","600","1200","900","1800","1200","2400","1500","3000","1700","3400","3400","6800","5100","10200","6800","13600","8500","17000","5200","10400","10400","20800","15600","31200","20800","41600","26000","52000"];

		// $net_premium_array = ["10338","14660","18050","20931","13728","16694","10847","15423","18982","22033","14406","17457","11270","16101","19830","23050","14999","18135","14321","20508","24406","27795","18220.34","22371","15423","22033","26270","29999","19660","23897","16524","23643","28135","32118","21016","25423","1695","3390","2542.372881","5085","3390","6780","4491","6355","7542","8643","5677","6270","2372","3135","3474","3728","2711","2965","4067","5677","6694","7626","5084","5592","3558","4914","5762","6525","4406.77","4830","9745","13897","15847","17711","11694","13558","5084","7118","7966","8728","5932","6694","8643","12287","13982","15592","10338","12033","7541","10762","12118","13389","8897","10507","6779.662","9745.763","12288.136","14406.78","9322.034","11864.407","8898.306","12288.136","15677.967","18220.34","11864.407","15254.238","9322.034","13559.323","16525.424","19915.255","13050.848","15932.204","9745.763","13983.051","17372.882","20254.238","13474.577","16101.695","10084.746","14406.78","17796.611","20677.967","13559.323","16525.424","7203.39","10169.492","12711.865","14830.509","9745.763","12288.136","9322.034","12711.865","16101.695","18644.068","12288.136","15677.967","11016.95","14830.509","18644.068","22457.628","14406.78","18220.34","12711.865","17372.882","22033.899","25847.458","16949.153","21610.17","13983.051","19491.526","24237.289","27627.119","18220.34","22203.39","254.2372881","508.4745763","508.4745763","1016.949153","762.7118644","1525.423729","1016.949153","2033.898305","1271.186441","2542.372881","1440.677966","2881.355932","2881.355932","5762.711864","4322.033898","8644.067797","5762.711864","11525.42373","7203.389831","14406.77966","4406.779661","8813.559322","8813.559322","17627.11864","13220.33898","26440.67797","17627.11864","35254.23729","22033.89831","44067.79661"];

		$premium_array = ["7605", "9931", "12438", "14489", "9867", "12799", "15903", "18443"];

		$net_premium_array = ["6444.92", "8416.10", "10540.68", "12278.81", "8361.86", "10846.61", "13477.12", "15629.66"];

		// echo count($premium_array);
		// echo '<br>';
		// echo count($net_premium_array);
		// exit;

		$family_construct_age_wise_si = $this
		->db
		->select("*")
		->from("family_construct_age_wise_si")
		// ->where("policy_detail_id IN (473,474,475,465,466,467)")
		->where("policy_detail_id IN (415)")
		->get()
		->result_array();
		...
		foreach($family_construct_age_wise_si as $single_row){
			$gross_premium = $single_row['PremiumServiceTax'];
			
			$key = array_search($gross_premium, $premium_array);
			echo $gross_premium;
			echo '<br>';
			echo $net_premium_array[$key];
			echo '<hr>';


			$update_net_premium = $this->db->set('net_premium_without_gst', $net_premium_array[$key]);
			$this->db->where('id',$single_row['id']);
			// $this->db->where('PremiumServiceTax',$gross_premium);
			$this->db->update('family_construct_age_wise_si');
		}

		$family_construct_wise_si = $this
		->db
		->select("*")
		...
		foreach($family_construct_wise_si as $single_row){
			$gross_premium = $single_row['PremiumServiceTax'];
			...
			$this->db->where('id',$single_row['id']);
			// $this->db->where('PremiumServiceTax',$gross_premium);
			$this->db->update('family_construct_wise_si');
		}



		$policy_creation_age = $this
		->db
		->select("*")
		->from("policy_creation_age")
		// ->where("policy_id IN (473,474,475,465,466,467)")
		->where("policy_id IN (415)")
		->get()
		->result_array();


		foreach($policy_creation_age as $single_row){
			$gross_premium = $single_row['premium_with_tax'];

			// if(empty($gross_premium)){
			// 	$gross_premium = $single_row['premium_with_tax'];
			// }
			...
			$update_net_premium = $this->db->set('net_premium_without_gst', $net_premium_array[$key]);
			$this->db->where('policy_creation_age_id',$single_row['policy_creation_age_id']);
			// $this->db->where('PremiumServiceTax',$gross_premium);
			$this->db->update('policy_creation_age');
		} 
