...
class Axis_redirection_data extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->productCode = "R05"; //Do not change this value here as this is mapped with database table
    ...
    public function index() {
        ...
        $this->load->view("redirection_view", compact('checksum', 'jsonData', 'encryptedJSONData'));
    }
            ...
            if ($checksum != $hashData) {
                ...
                if (empty($aJSONData)) {
                    ...
                    if ($isValid) {
                         ...
                         */
                        $resultLeadIDExistCheck = $this->db->select("lead_id,emp_id")
                        ...
                        if (!empty($resultLeadIDExistCheck)) {
							...
							$emp_id = $resultLeadIDExistCheck['emp_id'];
                ...
                if($quote_exp_check['status'] == 1){
					$message = $quote_exp_check['msg'];
					$this->load->retail_template('Retail/redirection_msgs', compact('message'));
					return false;
                  
                }
						// $quote_exp_check = $this->db->query("select emp_id from proposal where status = 'Quote Expired' and emp_id = '$emp_id'")->row_array();

						// if(count($quote_exp_check)>0)
						// {
						// $message = "Lead is expired, please create fresh lead to proceed";
						// $this->load->retail_template('Retail/redirection_msgs', compact('message'));
						// return false;
						...
						// }
                          ...
                          else{ $this->continue_with_existing_lead_auto($data);}
                         ...
                         */
                        $resultCombineCheck = $this->db->select("lead_id,emp_id")
                                ->from("employee_details ed")
                        ...
                        if (!empty($resultCombineCheck)) {
							...
							$emp_id = $resultCombineCheck['emp_id'];
                            $policy_parent_id = $this->getPolicyParentId();
                            $cust_data_json = json_encode($this->aFields);
...
//, "created_at" => date('Y-m-d H:i:s')
		...
		// if(count($quote_exp_check)>0)
		// {//print_pre($quote_exp_check);die;
			// $custFacingErrMsg = "Lead is expired, please create fresh lead to proceed";
			//echo 1;
            // $this->load->view('Retail/redirection_msgs', compact('custFacingErrMsg'), 'custFacingErrMsg');
    ...
    function is_valid_data() {
        ...
        /* foreach ($mandatoryCustomerFacingFields as $field){
          if (!(isset($this->$this->aFields[$field]) && trim($this->aFields[$field]) !== "")){
        ...
        if (!empty($message) || !empty($aCombineErr) || !empty($dobErrMsg) || !empty($otherErrMsg)) {
			...
			$arrInvalidData = array('product_id'=>'R05', 'lead_id'=>$this->aFields['crm_lead_id'], 
    ...
    function continue_lead_data() {
        ...
        if (!empty($this->input->post('cust_data_json')) && !empty($this->input->post('create_new'))) {
            ...
            if ($isCreateNew == 'yes') {
                ...
                if($quote_exp_check['status'] == 1){
                    ...
                    $redirectLink = base_url('Axis_redirection_post_data');
                    $data = array(
                        "status" => "2",
                        "url" => $redirectLink,
                    );
        ...
        } elseif (0) {//!empty($this->session->flashdata('flashdata_lead_continue'))){
            /* 	user will come when passing values like leadid already exist or customer_name, 
              mobile_number, product_name are already exist in database.
             */
    ...
    function redirect_to_dropoff_page($lead_id, $policy_id, $dropff_return_json = 0) {
        ...
        /* AND ua.status = 1 
          $sqlStr = "select ed.json_qote,ed.mob_no,ed.email,ed.lead_id,ua.type,ed.emp_id,pmws.policy_parent_id
          from employee_details as ed,user_activity as ua,product_master_with_subtype as pmws
          where ed.emp_id = ua.emp_id
          AND ed.product_id = pmws.product_code
          AND ua.emp_id = $emp_id "; */
        ...
        if (!empty($activityType)) {
            ...
            switch ($activityType) {
                ...
                case ($activityType == 2 || $activityType == 3):
                    ...
                    break;
                case 4:
                    $arr['link'] = base_url('api/payment_redirection');
                    break;
                case ($activityType == 5 || $activityType == 6):
                    $arr['link'] = base_url('payment_success_view_call/'.$emp_id_encrypt);
                    break;
            }
    ...
    function dropoff_journey($emp_id_encrypt,$send_type) {
        ...
        $val = $this->db->query("select ed.json_qote,ed.mob_no,ed.email,ed.lead_id,ua.type,ed.emp_id,pmws.policy_parent_id from employee_details as ed,user_activity as ua,product_master_with_subtype as pmws where ed.emp_id = ua.emp_id AND ed.product_id = pmws.product_code AND ua.status = 1 AND ua.emp_id = '$emp_id'")->row_array();


        $aD2CSession['d2c_session'] = array('emp_id' => $emp_id_encrypt, 'policy_parent_id' => $val['policy_parent_id'], 'dropoff' => true);
	...
	setcookie('CRM_Lead_Id', $val['lead_id'], 0);
        ...
        if ($val['type'] == 1) {
            ...
            $arr['link'] = base_url('api/payment_redirection');
            $this->logs_docs_insert($lead_id, $arr, $logs_type);
            redirect(base_url('api/payment_redirection'.$utm_string));
        } else {
            $arr['link'] = base_url('payment_success_view_call/'.$emp_id_encrypt);
            $this->logs_docs_insert($lead_id, $arr, $logs_type);

            redirect(base_url('payment_success_view_call/'.$emp_id_encrypt.''.$utm_string));
        }
	}
    }

    function logs_docs_insert($lead_id, $request, $type, $response = "") {
        $request_arr = ["lead_id" => $lead_id, "req" => json_encode($request), "type" => $type, "product_id" => 'R05'];
	...
	public function payment_success_view($emp_id_encrypt)
	{
		...
		if($query){
			...
			$request_arr = ["lead_id" => $query['lead_id'], "req" => $encrypted ,"res"=>$decrypted , "type"=>"payment_response_post", "product_id"=> "R05"];
  	        $this->db->insert("logs_docs",$request_arr);
