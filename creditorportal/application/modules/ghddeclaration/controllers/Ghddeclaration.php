...
class Ghddeclaration extends CI_Controller 
{
	...
	function index()
	{
		...
		$this->load->view('ghddeclaration/index',$result);
	...
	function fetch()
	{
		...
		$userListing = curlFunction(SERVICE_URL.'/api/ghdDeclarationListing',$_GET);
		...
		if(!empty($userListing['Data']['query_result']) && count($userListing['Data']['query_result']) > 0)
		{
			for($i=0;$i<sizeof($userListing['Data']['query_result']);$i++)
			{
				...
				if(in_array('UserEdit',$this->RolePermission)){
					$actionCol .='<a href="ghddeclaration/addEdit?text='.rtrim(strtr(base64_encode("id=".$userListing['Data']['query_result'][$i]['declaration_id'] ), '+/', '-_'), '=').'" title="Edit"><span class="spn-9"><i class="ti-pencil"></i></span></a>';
				}
				if(in_array('UserDelete',$this->RolePermission)){
					if($userListing['Data']['query_result'][$i]['is_active'] == 1){
						$actionCol .='&nbsp;&nbsp;<a href="javascript:void(0);" onclick="deleteData(\''.$userListing['Data']['query_result'][$i]['declaration_id'] .'\');" title="Delete"><span class="spn-9"><i class="ti-trash" style="color: red;"></i></span></a>';
					}
				}
	...
	function addEdit($id=NULL)
	{
		...
		$data['utoken'] = $_SESSION['webpanel']['utoken'];
		$data['id'] = $record_id;
		$getRecordDetails = curlFunction(SERVICE_URL.'/api/getGHDDeclarationDetails',$data);
		...
		$this->load->view('template/header.php');
		$this->load->view('ghddeclaration/addEdit',$result);
		$this->load->view('template/footer.php');
	}
	...
	function submitForm()
	{
		...
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
		{
			$data = array();
			$data['utoken'] = $_SESSION['webpanel']['utoken'];
			$data['declaration_id'] = (!empty($_POST['declaration_id'])) ? $_POST['declaration_id'] : '';
			...
			$addEdit = curlFunction(SERVICE_URL.'/api/addEditGHDDeclaration',$data);
	...
	function delRecord($id)
	{
		...
		$data['id'] = $id;
		$data['utoken'] = $_SESSION['webpanel']['utoken'];
		$delRecord = curlFunction(SERVICE_URL.'/api/delGHDDeclaration',$data);
