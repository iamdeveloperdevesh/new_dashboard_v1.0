...
function curlFunction($url, $post_fiels = null, $is_arr = false,$content_type=false)
{
	//echo "Url: ".$url."<br/>";
	//echo "Curl data: <pre>";print_r($post_fiels);
	if(!$is_arr){
		$str = http_build_query($post_fiels);
	}else if(!empty($content_type) && $content_type == 'application/json'){
		$str = json_encode($post_fiels);
	}else{
		$str = $post_fiels;
	}
...
function curlLsqFunction($url, $post_fiels = null, $is_arr = false,$content_type=false,$method='POST')
{
	$curl = curl_init();
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_POSTFIELDS =>json_encode($post_fiels),
