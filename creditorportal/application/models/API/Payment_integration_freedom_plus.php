...
class Payment_integration_freedom_plus extends CI_Model
{
    ...
    function acknowledgement_download_m($lead_id = '')
    {
        ...
        $data = $this
            ->db
            ->query("select ed.emp_id,ed.lead_id,mps.ABHI_sub_type_name,apr.certificate_number,ed.emp_firstname,ed.emp_lastname,ed.AXISBANKACCOUNT,apr.start_date,apr.end_date,apr.ReceiptNumber,p.premium,pd.txndate,mpst.policy_subtype_id,mpst.product_code,p.multi_status,mpst.HB_policy_type from employee_policy_detail AS epd,product_master_with_subtype AS mpst,master_policy_sub_type mps,employee_details as ed,proposal as p,api_proposal_response as apr,payment_details as pd where epd.product_name = mpst.id and mpst.policy_subtype_id = epd.policy_sub_type_id and p.policy_detail_id=epd.policy_detail_id and mps.policy_sub_type_id = mpst.policy_subtype_id and ed.emp_id = p.emp_id and p.id = pd.proposal_id and p.proposal_no = apr.proposal_no_lead and p.emp_id = apr.emp_id and p.status in('Success','Payment Received') and apr.certificate_number IS NOT NULL and ed.lead_id = '" . $lead_id . "' GROUP BY apr.certificate_number ORDER BY mpst.policy_subtype_id asc ")->result_array();

        $product_id = $this->db->get_where("employee_details",["lead_id" => $lead_id])->row_array()['product_id'];
        if($product_id == 'R10'){
            $q = "SELECT sum(premium) as premium from proposal where emp_id = '".$data[0]['emp_id']."'";
            $premium = $this->db->query($q)->row_array()['premium'];
            $data[0]['premium'] = $premium;
        }
	...
	//print_pre($data);exit;
        ...
        //print_pre($policy_detail_id_arr);exit; 
    ...
    if ($data['proposal_data']['product_code'] == 'R10'){
        $GPA_SI = $data['proposal_data']['sum_insured'];
        $GCI_SI = $this->db->get_where("proposal",["emp_id" => $data['proposal_data']['emp_id'],"policy_detail_id !="=> $data['proposal_data']['policy_detail_id']])->row_array()["sum_insured"];
        if($GCI_SI){
            $GFB_data = $this
                        ->db
                        ->query("select * from master_group_code where gci_group = '" . $GCI_SI . "' and si_group = '" . $GPA_SI . "' and family_construct = '" . $data['proposal_data']['familyConstruct'] . "' and product_code = '".$data['proposal_data']['product_code']."'")->row_array();
                    
                    $data['proposal_data']['group_code'] = $GFB_data['group_code'];
                    $data['proposal_data']['spouse_group_code'] = $GFB_data['spouse_group_code'];
        }
    }
...
//echo "in";exit;
		...
		// Combine COI logic
                ...
                //print_r($data);
//echo $collection_amt['pay_amt'];exit;
            }
            else
            {
...
if($unique_ref_no =='1627561047GPR03'){
echo	$unique_ref_no = '1627561047GPR03_2';
}
...
if($data['proposal_data']['product_code'] == 'R10'){
                ...
                else
                {
                    ...
                    if ($errorObj['ErrorNumber'] == '00' || ($errorObj['ErrorNumber'] == '302' && $data['proposal_data']['master_policy_no'] == $newArr['policyDtls']['PolicyNumber']))
                    {
                        ...
                        $api_insert = array(
                            ...
                            "CustomerID" => empty($newArr['policyDtls']['MemberCustomerID'])? $unique_ref_no : '',//$newArr['policyDtls']['MemberCustomerID'],
                            "MemberCustomerID" => empty($newArr['policyDtls']['MemberCustomerID'])? $unique_ref_no : '',
    ...
    function real_pg_check($lead_id)
    {
        ...
        $query = $this
            ->db
            ->query("SELECT ed.lead_id,ed.emp_id,product_id,is_non_integrated_single_journey FROM employee_details as ed where ed.lead_id ='" . $lead_id."'")->row_array();
...
if($query['is_non_integrated_single_journey'] == 1){//ankita single link journey changes
            ...
            $url = "https://pg-abhi.adityabirlahealth.com/PGMANDATE/service/api/enquirePayment";
            $fqrequest = array(
                "signature" => $CKS_value,
                "Source" => "AX",
                "Vertical" => $vertical,
			...
			if(isset($api_response_tbl['status']) && $api_response_tbl['status'] == 'Success'){
				...
				->where_in('id', $combiArr['id']);
                            $this
                                ->db
                                ->update("proposal", $arr);
$request_arr_status = ["lead_id" => $CRM_Lead_Id,"req" => json_encode($arr),"res" =>'',"product_id" => $update_payment['product_code'],"type"=> "proposal_status"];
$dataArray['tablename'] = 'logs_docs';
$dataArray['data'] = $request_arr_status;
$this->Logs_m->insertLogs($dataArray);
