...
class Payment_integration_freedom_plus extends CI_Model
{
    ...
    function get_all_quote_call($emp_id)
    {
        ...
        $get_data = $this
            ->db
            ->query('SELECT ed.emp_id,mpst.policy_subtype_id,p.policy_detail_id,p.id,mpst.HB_policy_type,epd.combine_coi FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.id AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND ed.emp_id="' . $emp_id . '" AND epd.combine_coi = 0 UNION ALL SELECT ed.emp_id,GROUP_CONCAT(mpst.policy_subtype_id ORDER BY mpst.policy_subtype_id) as policy_subtype_id,GROUP_CONCAT(p.policy_detail_id ORDER BY mpst.policy_subtype_id) as policy_detail_id, GROUP_CONCAT(p.id ORDER BY mpst.policy_subtype_id) as id,mpst.HB_policy_type,epd.combine_coi FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.id AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND ed.emp_id="' . $emp_id . '" GROUP BY epd.combine_coi HAVING epd.combine_coi != 0')->result_array();
        ...
        foreach ($get_data as $vall)
        {
            ...
            if ($vall['HB_policy_type'] == 'ProposalWise')
            {
                ...
                $query_check_ghi = $this
                    ->db
                    ->query("select id from ghi_quick_quote_response where policy_subtype_id = '" . $vall['policy_subtype_id'] . "' and emp_id='$emp_id' and proposal_id = '" . $vall['id'] . "' and status = 'success'")->row_array(); // Check this logic
            ...
            {
                $policyDetailId = explode(',',$vall['policy_detail_id']);
                $member_data = (array)$this->get_all_member_data($emp_id, $policyDetailId[0]);

                $query_start = $this
                    ->db
                    ->query("select count(id) as total_success from ghi_quick_quote_response where policy_subtype_id = '" . $vall['policy_subtype_id'] . "' and emp_id='$emp_id' and proposal_id = '" . $vall['id'] . "' and status = 'success'")->row_array(); // Check this logic
                ...
                if ($check_status == 'error')
                {
                    foreach ($member_data as $key => $value)
                    {
                        ...
                        $is_data = $this
                            ->db
                            ->query("select * from ghi_quick_quote_response where emp_id='$emp_id' and policy_subtype_id = '" . $value['policy_subtype_id'] . "' and proposal_id = '" . $vall['id'] . "' and fr_id = '" . $value['fr_id'] . "' ")->row_array(); // Check this logic
                        ...
                        if ($is_data)
                        {
                            ...
                            $query_check = $this
                                ->db
                                ->query("select * from ghi_quick_quote_response where emp_id='$emp_id' and policy_subtype_id = '" . $value['policy_subtype_id'] . "' and proposal_id = '" . $vall['id'] . "' and fr_id = '" . $value['fr_id'] . "' and status = 'error'")->row_array(); // Check this logic

                            if ($query_check)
                            {
                                $policy_data = $this->get_quote_data($emp_id, $vall['policy_detail_id'], $mem_data);
                            }
    ...
    function get_all_member_data_combined($emp_id, $policy_detail_id_arr)
    {
        $response = $this
            ->db
            ->query('SELECT result.*, SUM(result.policy_mem_sum_premium) AS combined_premium, GROUP_CONCAT(result.policy_subtype_id) AS combined_subtype_id FROM (SELECT epm.family_relation_id,e.policy_subtype_id,epd.policy_detail_id,epm.policy_mem_sum_premium,epd.suminsured_type,epm.policy_mem_sum_insured,epm.policy_member_id,"Self" AS "relationship",epm.policy_member_first_name AS "firstname", epm.policy_member_last_name AS "lastname",epm.fr_id AS "fr_id",epm.familyConstruct AS "familyConstruct",epm.policy_mem_gender AS "gender",epm.policy_mem_dob AS "dob", epm.age AS "age",epm.age_type AS "age_type","R001" AS relation_code,e.plan_code
                FROM product_master_with_subtype AS e,employee_policy_detail AS epd,proposal_member AS epm,family_relation AS fr,employee_details AS ed
                WHERE epd.product_name = e.id AND e.policy_subtype_id = epd.policy_sub_type_id AND epd.policy_detail_id = epm.policy_detail_id AND epm.family_relation_id = fr.family_relation_id AND fr.family_id = 0 AND fr.emp_id = ed.emp_id AND ed.emp_id = "'.$emp_id.'" AND `epd`.`policy_detail_id` IN ('.$policy_detail_id_arr.') UNION ALL
                SELECT epm.family_relation_id,e.policy_subtype_id,epd.policy_detail_id,epm.policy_mem_sum_premium,epd.suminsured_type,epm.policy_mem_sum_insured,epm.policy_member_id,mfr.fr_name AS "relationship",epm.policy_member_first_name AS "firstname", epm.policy_member_last_name AS "lastname",epm.fr_id AS "fr_id",epm.familyConstruct AS "familyConstruct",epm.policy_mem_gender AS "gender", epm.policy_mem_dob AS "dob",epm.age AS "age",epm.age_type AS "age_type",mfr.relation_code,e.plan_code
                FROM product_master_with_subtype AS e,employee_policy_detail AS epd,proposal_member AS epm,family_relation AS fr,employee_family_details AS efd,master_family_relation AS mfr
                WHERE epd.product_name = e.id AND e.policy_subtype_id = epd.policy_sub_type_id AND epd.policy_detail_id = epm.policy_detail_id AND epm.family_relation_id = fr.family_relation_id AND fr.family_id = efd.family_id AND efd.fr_id = mfr.fr_id AND `epd`.`policy_detail_id` IN ('.$policy_detail_id_arr.') AND fr.emp_id = "'.$emp_id.'") AS result GROUP BY result.fr_id')->result_array();
    ...
    {
        $policyDetailIdArr = explode(',',$policy_detail_id);
        $member_data = (array)$this->get_all_member_data($emp_id, $policyDetailIdArr[0]);
        $member_data = (array)$this->get_all_member_data_combined($emp_id, $policy_detail_id);
        $query = $this
            ->db
            ->query("select multi_status from proposal where emp_id='$emp_id' and policy_detail_id IN (".$policy_detail_id.")")->row_array();
        ...
        if (empty($query['multi_status']))
        {
            ...
            $this
                ->db
                ->where_in("policy_detail_id",$policyDetailIdArr);
            $this
                ->db
                ->update("proposal", $arr);
        }
        ...
        $query_last = $this
            ->db
            ->query("select multi_status from proposal where emp_id='$emp_id' and policy_detail_id IN (".$policy_detail_id.")")->row_array();
        // print_pre($member_data[0]['familyConstruct']);
        ...
        return $return_data = array(
            'status' => $proposal_status,
            //added by upendra on 15-05-2021
            'GrossPremium' => $member_data[0]['combined_premium'],
            "msg" => "GPA GCI GP call"
        );
    ...
    function get_quote_data($emp_id, $policy_detail_id, $mem_data = '')
    {
        ...
        //replaced cust_id with unique_ref_no on 15-05-2021 by upendra
        ...
        // echo $data['proposal_data']['product_code'].' '.$new_check_data['lead_id'].' '.$data['proposal_data']['policy_sub_type_id'];exit;
        ...
        // echo $data['proposal_data']['HB_policy_type'];exit;
        ...
        // Plan Code & Sum Insured for combined COI In R10
        ...
        // print_pre($planCodeSi);
        // print_pre($memberObjPlanCode);
        // Plan Code & Sum Insured for combined COI In R10
		...
		$occupation = "O553";
		if ($data['proposal_data']['product_code'] == 'R07' || $data['proposal_data']['product_code'] == 'R11' || $data['proposal_data']['product_code'] == 'R10'){
        ...
        for ($i = 0;$i < $totalMembers;$i++)
        {
            ...
            //added by upendra on 24-05-2021 (memberwise remark logic)
			$memberPEDARRAY = [];
			if ($data['proposal_data']['product_code'] == 'R03' || $data['proposal_data']['product_code'] == 'R07' || $data['proposal_data']['product_code'] == 'R11'){
...
// print_pre($abc);exit;
        ...
        //ankita single link journey pass terminal id so that pg wil show only Axis Bank Payment mode
        $imdBranchCode = "10MHMUM01";
        $terminal_id = '';
        $panNo = null;
        $idProof = "Adhaar Card";
        if($data['customer_data']['is_non_integrated_single_journey'] == 1){
            $terminal_id = 'H6bzurUIQggI2Z';
            if($data['customer_data']['ref1'] == 0 || $data['customer_data']['ref1'] == ""){
                $data['customer_data']['ref1'] = $data['customer_data']['emp_code'];
                $data['customer_data']['ref2'] = $data['customer_data']['emp_code'];
            }
            if($data['customer_data']['pancard'] != ""){
                $panNo = $data['customer_data']['pancard'];
            }
            $idProof = null;
            if($data['customer_data']['branch_id'] == ""){
                $data['customer_data']['branch_id'] = "NULL";
            }
            $imdBranchCode = "10MHMUM08";
        }
                    

        //print_pre($member);exit;
        $fqrequest = ["ClientCreation" => ["Member_Customer_ID" => $unique_ref_no, "salutation" => $data['customer_data']['salutation'], "firstName" => $data['customer_data']['emp_firstname'], "middleName" => "", "lastName" => !empty(trim($data['customer_data']['emp_lastname'])) ? $data['customer_data']['emp_lastname'] : '.', "dateofBirth" => date('m/d/Y', strtotime($data['customer_data']['bdate'])) , "gender" => ($data['customer_data']['gender'] == "Male") ? "M" : "F", "educationalQualification" => null, "pinCode" => $data['customer_data']['emp_pincode'], "uidNo" => $data['customer_data']['adhar'], "maritalStatus" => null, "nationality" => "Indian", "occupation" => $occupation, "primaryEmailID" => $data['customer_data']['email'], "contactMobileNo" => substr(trim($data['customer_data']['mob_no']) , -10) , "stdLandlineNo" => null, "panNo" => $panNo, "passportNumber" => null, "contactPerson" => null, "annualIncome" => $AnnualIncome, "remarks" => null, "startDate" => date('Y-m-d') , "endDate" => null, "IdProof" => $idProof, "residenceProof" => null, "ageProof" => null, "others" => null, "homeAddressLine1" => $data['customer_data']['address'], "homeAddressLine2" => null, "homeAddressLine3" => null, "homePinCode" => $data['customer_data']['emp_pincode'], "homeArea" => null, "homeContactMobileNo" => null, "homeContactMobileNo2" => null, "homeSTDLandlineNo" => null, "homeFaxNo" => null, "sameAsHomeAddress" => "1", "mailingAddressLine1" => null, "mailingAddressLine2" => null, "mailingAddressLine3" => null, "mailingPinCode" => null, "mailingArea" => null, "mailingContactMobileNo" => null, "mailingContactMobileNo2" => null, "mailingSTDLandlineNo" => null, "mailingSTDLandlineNo2" => null, "mailingFaxNo" => null, "bankAccountType" => null, "bankAccountNo" => null, "ifscCode" => null, "GSTIN" => null, "GSTRegistrationStatus" => "Consumers", "IsEIAavailable" => "0", "ApplyEIA" => "0", "EIAAccountNo" => null, "EIAWith" => "0", "AccountType" => null, "AddressProof" => null, "DOBProof" => null, "IdentityProof" => null], "PolicyCreationRequest" => ["Quotation_Number" => "", "MasterPolicyNumber" => $data['proposal_data']['master_policy_no'], "GroupID" => $data['proposal_data']['group_code'], "Product_Code" => $data['proposal_data']['plan_code'], "SumInsured_Type" => null, "Policy_Tanure" => "1", "Member_Type_Code" => "M209", "intermediaryCode" => $data['proposal_data']['IMDCode'], "AutoRenewal" => $AutoRenewalFlag, "AutoDebit" => $AutoRenewalFlag, "intermediaryBranchCode" => $imdBranchCode, "agentSignatureDate" => null, "Customer_Signature_Date" => null, "businessSourceChannel" => null, "AssignPolicy" => "0", "AssigneeName" => null, "leadID" => $data['customer_data']['lead_id'], "Source_Name" => $source_name, "SPID" => $data['customer_data']['ref2'], "TCN" => null, "CRTNO" => null, "RefCode1" => $data['customer_data']['ref1'], "RefCode2" => $data['customer_data']['branch_id'], "Employee_Number" => $data['proposal_data']['emp_id'], "enumIsEmployeeDiscount" => null, "QuoteDate" => null, "IsPayment" => "0", "PaymentMode" => null, "PolicyproductComponents" => $planCodeSi], "MemObj" => ["Member" => $member], "ReceiptCreation" => ["officeLocation" => "", "modeOfEntry" => "", "cdAcNo" => null, "expiryDate" => null, "payerType" => "", "payerCode" => null, "paymentBy" => "", "paymentByName" => null, "paymentByRelationship" => null, "collectionAmount" => "", "collectionRcvdDate" => null, "collectionMode" => "", "remarks" => null, "instrumentNumber" => null, "instrumentDate" => null, "bankName" => null, "branchName" => null, "bankLocation" => null, "micrNo" => null, "chequeType" => null, "ifscCode" => "", "PaymentGatewayName" => "", "TerminalID" => $terminal_id, "CardNo" => null]];
        // print_pre(json_encode($fqrequest, JSON_PRETTY_PRINT));exit;
    ...
    function GHI_GCI_api_call($emp_id, $policy_no, $mem_data = '', $field_array = '', $cron_policy_check = '')
    {
       ...
       $extra_check_data = $this
        ...
        if ((($extra_check_data['payment_status'] == 'Success' && !empty($extra_check_data['EasyPayId'])) || ($extra_check_data['payment_status'] == 'No Error' && !empty($extra_check_data['TxRefNo']) && $extra_check_data['TxStatus'] == 'success')) && $extra_check_data['is_policy_issue_initiated'] == 0)
        {
            ...
            $extra_arr_update = ["is_policy_issue_initiated" => 1]; //Test Purpose change to 1 again
            $this
                ->db
                ->where("emp_id", $emp_id);
            $this
            ...
            if ($data['proposal_data']['HB_policy_type'] == 'ProposalWise')
            {
				...
				if ($data['proposal_data']['product_code'] == 'R03' && !empty($new_check_data['lead_id']) && $data['proposal_data']['policy_sub_type_id'] == 1)
				{
                    //replaced cust_id with uniue_ref_no on 15-05-2021 - by upendra
					// $cust_id = $data['customer_data']['cust_id'].'GHIR03';
					$unique_ref_no = $data['customer_data']['unique_ref_no'].'GHIR03';
				}
            ...
            else
            {
                ...
                if($data['proposal_data']['product_code'] == 'R10'){
                    $query_quote = $this
                    ->db
                    ->query("select * from ghi_quick_quote_response where emp_id='$emp_id' and policy_subtype_id IN (" . $mem_data[0]['combined_subtype_id'] . ") and fr_id = '" . $mem_data[0]['fr_id'] . "' and status = 'success'")->row_array();
                }
                ...
                $collection_amt['pay_amt'] = $data['member_data'][0]['combined_premium'];
            ...
            if ($query_quote['QuotationNumber'])
            {
                ...
                for ($i = 0;$i < $totalMembers;$i++)
                {
                 ...
                 $check_full_quote = "select id,req from logs_docs where type = 'full_quote_request2_".$data['proposal_data']['policy_sub_type_id']."
' and created_at >= '".$date."'";

                                $check_full_quote = $this->db->query($check_full_quote)->row_array();

                                if(!empty($check_full_quote)){
                                        $mem_cust_num = json_decode($check_full_quote['req'],true);
                                        if($mem_cust_num['Member_Customer_ID'] == $unique_ref_no){
                                        $log = ["lead_id" => $data['customer_data']['lead_id'], "req" => json_encode($check_full_quote) , "product_id
" => $data['proposal_data']['product_code'], "type" => "check_fail_quote"];
                                        $this
                    ->db
                    ->insert("logs_docs", $log);
                ...
                else
                {
                    ...
                    if ($errorObj['ErrorNumber'] == '00' || ($errorObj['ErrorNumber'] == '302' && $data['proposal_data']['master_policy_no'] == $newArr['policyDtls']['PolicyNumber']))
                    {
						...
						if(empty($cert_check)){
    						if($data['proposal_data']['product_code'] == 'R10'){     // Will work for ProposalWise Combine COI
                                foreach ($policy_detail_id_arr as $key => $policyDetailId) {
                                    ...
                                    $api_insert["proposal_id"] = $proposalRecord['id'];
                                    $api_insert["proposal_no_lead"] = $proposalRecord['proposal_no'];
    ...
    function send_message($lead_id, $type)
    {
        ...
        if ($query_check)
        {
            ...
            $senderID = 1;
            $full_name = $query_check['emp_firstname'] . " " . $query_check['emp_lastname'];
    ...
    function policy_creation_call($CRM_Lead_Id, $cron_policy_check = '')
    {
        ...
        $update_data = $this->db->query('SELECT ed.emp_id,mpst.policy_subtype_id,p.policy_detail_id,p.id,p.status,p.count,mpst.HB_policy_type,mpst.product_code,epd.combine_coi FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.id AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND ed.lead_id="' . $CRM_Lead_Id . '" AND epd.combine_coi = 0 UNION ALL SELECT ed.emp_id,GROUP_CONCAT(mpst.policy_subtype_id ORDER BY mpst.policy_subtype_id) as policy_subtype_id,GROUP_CONCAT(p.policy_detail_id ORDER BY mpst.policy_subtype_id) as policy_detail_id, GROUP_CONCAT(p.id ORDER BY mpst.policy_subtype_id) as id,GROUP_CONCAT(p.status ORDER BY mpst.policy_subtype_id) as status,GROUP_CONCAT(p.count ORDER BY mpst.policy_subtype_id) as count,mpst.HB_policy_type,mpst.product_code,epd.combine_coi FROM employee_policy_detail AS epd,product_master_with_subtype AS mpst,employee_details AS ed,proposal as p WHERE p.emp_id = ed.emp_id and epd.product_name = mpst.id AND mpst.policy_subtype_id = epd.policy_sub_type_id AND p.policy_detail_id=epd.policy_detail_id AND ed.lead_id="' . $CRM_Lead_Id . '" GROUP BY epd.combine_coi HAVING epd.combine_coi != 0');

        $update_data = $update_data->result_array();
//Dedupe - sonal
$emp_id = $update_data[0]['emp_id'];
			$query_lead = $this
            ->db
            ->query('SELECT emp_id
...
//Reverse feed - sonal
  $quote_exp_check = common_quote_expired($emp_id);
                if($quote_exp_check['status'] == 1){
                  echo   $quote_exp_check['msg'];exit;
                }

        // print_pre("Group by combine_coi");
        ...
        foreach ($update_data as $update_payment)
        {
            ...
            //update payment confirmation hit count // Commented for test purpose
            ...
            if ($combiArr['status'][0] != 'Success')
            {
                ...
                if($combiArr['count'][0] < 3)
                {
                    ...
                    $this
                        ->db
                        ->where_in('emp_id', $update_payment['emp_id']);
                    $this
                        ->db
                        ->update("employee_details", $arr_new_lead);
                    ...
                    if ($query)
                    {
                        ...
                        else
                        {
                            ...
                            $this
                                ->db
                                ->update("employee_details", $arr_lead_update);
    ...
    function coi_download_m($certificate_no = '')
    {
        ...
        if ($result['status'] == 'error')
        {
            ...
            $new_url = $this
                ->db
                ->query("select url from coi_genarate_url where lead_id = '" . $data['lead_id'] . "' ")->row_array();

            if ($new_url['url'] == "false" || $new_url['url'] == "")
            {
                $new_download_method = $this->acknowledgement_download_m($data['lead_id']);

                if ($new_download_method['status'] == 'success')
                {
                    $result = array(
                        "status" => "success",
                        "url" => $new_download_method['url']
                    );
                }
            }
            else
    ...
    function acknowledgement_download_m($lead_id = '')
    {
        ...
        foreach ($data as $t => $v)
        {
            ...
            if($v['product_code'] == 'R10'){
                $product_data[$t]['premium'] = $v['gross_premium'];  
            }
 ...
 public function fetch_policy_data_m()
        ...
        if (!empty($Source) && !empty($Vertical) && !empty($PaymentMode) && !empty($UniqueIdentifier) && !empty($UniqueIdentifierValue))
        {
            ...
            if (!empty($product_id) && $UniqueIdentifier == 'LEADID')
            {
                ...
                if ($product_id == 'R05' || $product_id == 'ABC')
                {
                    ...
                    $extra_check = " and ed.branch_sol_id = imd.BranchCode";
                    if($product_id == 'ABC'){
                        $extra_check = "";
                        //
                        $lead_data = $this->db1 ->query("select ed.acc_type,ed.acc_no,ed.lead_id,pd.TxRefNo,pd.txndate,ed.cust_id,sum(p.premium) as premium,ed.emp_id from employee_details as ed,proposal as p,payment_details as pd where ed.emp_id = p.emp_id and p.id = pd.proposal_id  and p.status in('Success','Payment Received') and ed.product_id = '" . $product_id . "' and ed.lead_id = '" . $UniqueIdentifierValue . "'")->row_array();
                    ...
                    if (!empty($lead_data['emp_id']))
                    {
                       ...
                       /* $policy_data = $this
                        ...
                        $policy_data = $this->db1->query("SELECT m.certificate_number,m.proposal_no,p.premium FROM proposal p,api_proposal_response m WHERE p.id=m.proposal_id and m.emp_id = '" . $lead_data['emp_id'] . "'")->result_array();
                        //print_r($policy_data);exit;
                        if (!empty($policy_data[0]['certificate_number']))
                        {
                            $check = 1;
                            $certificate_arr = array_column($policy_data, 'certificate_number');
                            $proposal_no_arr = array_column($policy_data, 'proposal_no');               
                        ...
                        if ($check)
                        {
                            ...
                            foreach($policy_data as $key => $val){
                                $res[$key] = array(
                                    "Bank_Name" => "Axis Bank",
    ...
    public function fetch_policy_data_m_bk()
    {
