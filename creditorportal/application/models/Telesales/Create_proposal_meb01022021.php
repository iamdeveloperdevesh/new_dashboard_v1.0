...
class Create_proposal_m extends CI_Model
{
	...
	public function add_agent_emp_details($emp_id,$emp_agent_data)
	{
		...
		//check spouse gender and update
...
$response = $this->db
        ->query('SELECT epm.policy_detail_id,epd.policy_sub_type_id,epm.family_relation_id,epm.policy_mem_sum_premium,epd.suminsured_type,epm.policy_mem_sum_insured,epm.policy_member_id,"self" AS "relationship",epm.policy_member_first_name AS "firstname", epm.policy_member_last_name AS "lastname",epm.fr_id as "fr_id",epm.familyConstruct as "familyConstruct",
            ...
            AND fr.emp_id = ed.emp_id
            AND ed.emp_id = ' . $emp_id . '
            AND `epd`.`policy_detail_id` = ' . $policy_detail_id . ' UNION all SELECT epm.policy_detail_id,epd.policy_sub_type_id,epm.family_relation_id, epm.policy_mem_sum_premium,epd.suminsured_type,epm.policy_mem_sum_insured,epm.policy_member_id,mfr.fr_name AS "relationship",epm.policy_member_first_name AS "firstname", epm.policy_member_last_name AS "lastname",epm.fr_id as "fr_id",epm.familyConstruct as "familyConstruct",
   ...
   public function edit_member_declare_data($emp_id, $emp_policy_mem,$subid)
	...
	public function get_common_data()
	{
		...
		$data['base_agent_details'] = $this->db->query("select ba.*,ed.new_remarks,ed.comm_address,ed.comm_address1,ed.emg_cno,ed.axis_lob,ed.axis_location,ed.axis_vendor from employee_details as ed,tls_base_agent_tbl as ba where ed.agent_id = ba.base_agent_id AND emp_id = '$emp_id'")->row_array();
...
public function policy_detail_not_combo($product_name)
{
    ...
    public function get_insured_summary($member_datas, $policy_sub_type_name, $parent_id, $emp_id,$plan_name,$master_policy)
    {
        ...
        foreach ($member_datas as $member_data)
        {
			...
			if($z == 1){
				$customer_detail .= '<div class="col-md-12"> <p class="mt-2 mb-2 text-center" style="color: #da8089;font-size: 17px;">'.$policy_sub_type_name .'</p><table class="table table-bordered text-center"><thead class="text-uppercase col-da80">';
			}else{
	...
	<th scope="col" style="font-weight:600 !important;width:25%;">' . $member_data['policy_mem_sum_insured'] . '</th>
	<th style="width: 1px;border: none !important;border-color: #fff;"></th>
                ...
                foreach ($subtype_id as $subid)
                {
                    ...
                    foreach ($policy_member_declare as $key => $value)
                    {
                        $customer_detail .= '<tr>
											...
											<td style="width: 150px;"><div class="custom-control custom-radio" style="float: left;"><input type="radio"  disabled name="' . $value['p_member_id'] . '" id="' . $value['p_member_id'] . '" class="custom-control-input radios_out" value="Yes" > <label class="custom-control-label" for="' . $value['p_member_id'] . '"> Yes </label> </div>
