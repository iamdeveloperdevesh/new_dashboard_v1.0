...
class Lead_management_m extends CI_Model
{
    public $agent_id;
    public $admin;
    ...
    // public $select_column = array("l.emp_id", "l.lead_id","IFNULL(l.emp_lastname, '') as 'last_name'", "l.emp_firstname", "'Group Activ Health-Tele' as 'Plan'", "IFNULL(p.premium, 0) as 'premium'", "if(p.status is null, IF(DATEDIFF(NOW(),l.created_at) > 15 , 'rejected', 'proposal not created'),p.status) as status", "IFNULL(p.modified_date, '') as 'modified_date'","IFNULL(p.sms_trigger_status, '') as sms_trigger_status", "l.mob_no", "l.saksham_id", "IFNULL(apr.certificate_number, '') as 'policy_no'","tam.id as 'agent_id'","tam.agent_name","l.created_at","IFNULL(p.proposal_no, '') as 'proposal_no'","IFNULL(l.av_remark, '') as 'av_remark'","IFNULL(l.preferred_contact_time, '') as 'preferred_contact_time'","l.product_id");
    ...
    public function make_query_old()
    {
        ...
        if (true) {
			$this->order_column = [];
			$this->order_column = array(null, "l.lead_id", "l.emp_firstname", "Plan", "p.premium", "p.status", "p.modified_date","p.sms_trigger_status", "l.mob_no", "l.saksham_id", "apr.certificate_number","tam.agent_name","l.created_at","l.axis_location");
          $xlfields_admin = ["tam.agent_id as 'av_code'","l.emp_firstname", "IFNULL(pd.txndate, '') as 'txndate'", "IFNULL(pd.TxStatus, '') as 'TxStatus'", "IFNULL(apr.start_date, '') as 'policystartdate'", "IFNULL(p.sum_insured, '')as 'sum_insured'", "l.bdate", "IFNULL(tam.tl_name, '') as 'tl_name'", "IFNULL(tam.am_name, '') as 'am_name'", "IFNULL(tam.om_name, '') as 'om_name'", "IFNULL(axis_location.axis_location, '') as 'axis_location'", "IFNULL(axis_lob.axis_lob, '') as 'axis_lob'","IFNULL(axis_vendor.axis_vendor, '') as 'axis_vendor'", "IFNULL(l.emp_city, '') as 'emp_city'", "IFNULL(l.emp_state, '') as 'emp_state'", "IFNULL(l.emp_pincode, '') as 'emp_pincode'", "l.mob_no", "IFNULL(l.auto_renewal, '') as 'businessType'", "IFNULL(edmst.emp_sub_type_id, 'NO') as 'Chronic'","IFNULL(l.email, '') as 'email'","IFNULL(l.preferred_contact_date, '') as 'preferred_contact_date'","IFNULL(l.agent_name, '') as 'base_caller_name'","IFNULL(l.agent_id, '') as 'base_caller_id'","tbat.imd_code"];
			$this->select_column = array_merge($this->select_column, $xlfields_admin);
        }
        $this->db->select($this->select_column);
        $this->db->from($this->table);
        ...
        if (true) {
           // $this->db->join('family_relation fr', 'l.emp_id = fr.emp_id');
            ...
            $this->db->join('employee_declare_member_sub_type edmst', 'l.emp_id = edmst.emp_id', 'left');
            $this->db->join('tls_base_agent_tbl tbat', 'l.agent_id = tbat.base_agent_id', 'left');
			$this->db->join('tls_axis_lob axis_lob', 'l.axis_lob = axis_lob.axis_lob_id', 'left');
            $this->db->join('tls_axis_location axis_location', 'l.axis_location = axis_location.axis_loc_id', 'left');
            $this->db->join('tls_axis_vendor axis_vendor', 'l.axis_vendor = axis_vendor.axis_vendor_id', 'left');
        }
    ...
    public function make_datatables2($code)
    {
        $this->make_query();
	...
	function getRandomUniqueNumber($length = 15) {
	 ...
	 public function generate_lead_id(){
		...
		if(!empty($leadcheck) || !empty($leadcheckaxis)){
    ...
    function axis_process(){
        
        $this->db->distinct()->select('axis_process');
        $this->db->from('tls_axis_lob');
        $this->db->order_by('axis_process','ASC');
    ...
    public function get_net_premium_tele_test($lead_id){
        $empData = $this->db->select('emp_id')
            ->from('employee_details')
            ->where('lead_id', $lead_id)
            ->get()
    ...
    public function get_quotation_no($lead_id){
        $lead_id = "9879814654";

        // $get_half_quote =  $this->db->select("res,id")
        // ->from("logs_docs")
        // ->where("lead_id", $lead_id)
        // ->where("type LIKE '%full_quote_request1_%'")
        // ->group_by("type")
        // ->order_by("id DESC")
        // ->get()
        // ->result_array();

        $get_half_quote =  $this->db->select("res")
        ->from("logs_docs")
        ->where("id", '43905')
        ->get()
        ->row_array();

        print_r($get_half_quote);
        // $new = simplexml_load_string($get_half_quote['res']);
        // $con = json_encode($new);
        ...
        $xml_com = simplexml_load_string($get_half_quote[0]['res']);
    ...
    public function dynamic_lob_load(){
        ...
        $this->db->select('DISTINCT(axis_lob) AS axis_lob,axis_lob_id,axis_process,axis_vendor_id,is_maker_checker,telesales_journey');
        $this->db->from('tls_axis_lob');
        $this->db->group_by('axis_lob');
        $this->db->order_by('axis_lob_id DESC');
        ...
        {  
             ...
             $this->db->group_start();
             $this->db->like('axis_lob', $_POST["search"]["value"]);  
    ...
    public function dynamic_lob_load_count(){

      
        $this->db->select('count(DISTINCT(axis_lob)) as ids');
        $this->db->from('tls_axis_lob');
        $this->db->order_by('axis_lob_id DESC');
