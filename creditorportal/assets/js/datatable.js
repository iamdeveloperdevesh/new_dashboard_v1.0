...
$(document).ready(function() { 	
	...
	if($('table').hasClass('dynamicTable')){
		...
		var custom_callfunction = $(".dynamicTable").attr("callfunction");	
		var fetch_url = "./"+filename+"/fetch?act=fetch";
		if(custom_callfunction != undefined){
			fetch_url = custom_callfunction;
		}
		oTable = $('.dynamicTable').dataTable({
			...
			"sAjaxSource": fetch_url,
			"fnInitComplete": function(oSettings, json) {
				if(oSettings.json.saleadmintotal != undefined){
					$('.dynamicTable').find(".saleadmintotal").html(oSettings.json.saleadmintotal);
				}
				if(oSettings.json.saleadmintotalwithtax != undefined){
					$('.dynamicTable').find(".saleadmintotalwithtax").html(oSettings.json.saleadmintotalwithtax);
				}

				if(oSettings.json.weeklyNetTot != undefined){
					$('.dynamicTable').find(".weeklyNetTot").html(oSettings.json.weeklyNetTot);
				}
				if(oSettings.json.weeklyGrossTot != undefined){
					$('.dynamicTable').find(".weeklyGrossTot").html(oSettings.json.weeklyGrossTot);
				}
				if(oSettings.json.mothlyNetTot != undefined){
					$('.dynamicTable').find(".mothlyNetTot").html(oSettings.json.mothlyNetTot);
				}
				if(oSettings.json.mothlyGrossTot != undefined){
					$('.dynamicTable').find(".mothlyGrossTot").html(oSettings.json.mothlyGrossTot);
				}
				if(oSettings.json.yearlyNetTot != undefined){
					$('.dynamicTable').find(".yearlyNetTot").html(oSettings.json.yearlyNetTot);
				}
				if(oSettings.json.yearlyGrossTot != undefined){
					$('.dynamicTable').find(".yearlyGrossTot").html(oSettings.json.yearlyGrossTot);
				}
				if(oSettings.json.dateRangeNetTot != undefined){
					$('.dynamicTable').find(".dateRangeNetTot").html(oSettings.json.dateRangeNetTot);
				}
				if(oSettings.json.dateRangeGrossTot != undefined){
					$('.dynamicTable').find(".dateRangeGrossTot").html(oSettings.json.dateRangeGrossTot);
				}
