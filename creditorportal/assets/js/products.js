...
$(document).on('click', '#add-another-member', function () {
    ...
    $('.adult-row').last().find('[name="maxage[]"]').attr('id','mem'+id);
    
    $('.adult-row').last().find('[name="minage[]"]').attr('id','memmin'+id);
...
$(document).on('click', '.delete-member-btn', function () {
    ...
    if (adult_row_count <= 1) {
        displayMsg("error", "Policy should have atleast one member type");
        return;
    }
...
function canAddMoreAdultMembers() {
    let adult_row_count = $('.adult-row').length;
    let member_count = parseInt($('#membercount').val());
