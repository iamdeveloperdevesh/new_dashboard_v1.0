		...
		if(value != 0){
			for (var i = 0; i < value; i++) {
				...
				$.each(childFamilyConstructs, function(key, childConstruct){
					...
					relationOptionsKid += "<option "+selected+" value='"+childConstruct.fr_id+"'>"+childConstruct.fr_name+"</option>";
				});
				dob_modal_data_kid += "<div class='col-md-6 form-group'><label class='col-form-label'>Dependent Kid "+(i+1)+"<sup class='star_sup'><img src='/public/assets_new/images/important.png'></sup></label><select class='form-control' name='relation["+(i+1)+"]' id='Kid"+(i+1)+"_relation' type='text' value='' placeholder='Select Dependent Kid "+(i+1)+"'><option value='' selected='selected'>Select Dependent Kid "+(i+1)+"</option>"+relationOptionsKid+"</select></div><div class='col-md-6'><div class='form-group'><label for='example-date-input' class='col-form-label'>Dependent Kid "+(i+1)+" Date Of Birth <sup><img src='/public/assets_new/images/important.png'></sup></label><input class='form-control datepicker' name='kid_dob["+(i+1)+"]' placeholder='DD-MM-YYYY' value='"+kidDob+"'><i class='fa fa-calendar'></i></div></div>";
			}
...
function occupationLogicBoxShowHide(){
	...
	} else if (adultsSelected.length == 2 || adultsSelected.length == 0) {
