    ...
    $.ajax({
        ...
        $.ajax({
            ...
            success: function (response) {                        
                ...
                if(response.dec1=='0'||response.dec2=='0'){
                    var dec1='no';
                    var dec2='no';  
                }else if(response.dec1=='1'||response.dec2=='1'){
                    var dec1='yes';
                    var dec2='yes';
                }
...
'application-id-abhi-insurance': $("#lead_id_hidden").val(),
    ...
    $(document).on('click','#declaration_form_submit',function(){
        ...
        //         return false;
        // }
        if( $("input[name='d2c2_typ1']:checked").length == 0 || $("input[name='d2c2_typ2']:checked").length == 0 ){
            go_ahead_proposal = false;
            sad_icon("In order to proceed further please answer the health related questions", false);
            return false;
        }
        if($("input[name='d2c2_typ1']:checked").val() == '1' || $("input[name='d2c2_typ2']:checked").val() == '1'){
            go_ahead_proposal = false;
