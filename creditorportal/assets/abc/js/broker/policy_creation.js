...
var obj1 = {};
    ...
    $(document).on('change', '#familyConstruct', function () {
        ...
        $.ajax({
            type: "POST",
            url: "/broker/get_family_relation",
            async: false,
        ...
        if (q[0] == 1 && q[1] == 0) {
            ...
            for (var i = 0; i <= e.length; i++) {
                ...
                if (e[i].fr_id == 0) {
                    $("#family_construct_relations ul").append("<li><input id='chk_" + e[i].fr_id + "' type='checkbox' name='family_cons_rel' value='" + e[i].fr_id + "' /><label class='lbl_chk' for='chk_" + e[i].fr_id + "'>" + e[i].fr_name + "</label></li>");
                }
            }
        } else if (q[0] == 2 && q[1] == 0) {
            $("#family_construct_relations ul").empty();
            for (var i = 0; i <= e.length; i++) {
                ...
                if (e[i].fr_id == 0 || e[i].fr_id == 1) {
                    $("#family_construct_relations ul").append("<li><label for='chk_" + e[i].fr_id + "'>" + e[i].fr_name + "</label><input id='chk_" + e[i].fr_id + "' type='checkbox' name='family_cons_rel' value='" + e[i].fr_id + "' /></li>");
                }
            }
        } 
    ...
    $("#policyType").change(function () {
        ...
        // data1.append("policy_type_id",$("#policyType").val());
        /* 
         
         $.ajax({
         type: "POST",
         url: "/broker/get_policy_subType",
         data:data1,
         async:false,
         success: function (e) {
         e = JSON.parse(e);
         ...
         $("#policySubType").empty();
         $("#policySubType").html("<option value=''>-</option>");
                            ...
                            closeOnConfirm: true
                                  ...
                                  $("#payment_mode").val(null).trigger('change'); 
								   $('#companySubTypePolicy').trigger("change");
									...
									$.ajax({
									type: "POST",
									url: "/broker/update_policy_with_product",
									data: {
...
$.ajax({
    type: "POST",
    url: "/broker/get_payment_mode_data",
    async: false,
    success: function (e) {
...
$.ajax({
    type: "POST",
    url: "/broker/get_customer_search_data",
    async: false,
    success: function (e) {
 ...
 $('#subtype').on("click", function () {
	...
	// return false;
	 ...
	 var product_name = $("#productName").val();
	    ...
	    $.ajax({
		url: "/broker/save_tem_data",
        type: "POST",
		data: {policy_subtypes_id:policy_subtypes_id,policy_types_id:policy_types_id,random_data:random_data,product_name:product_name},
