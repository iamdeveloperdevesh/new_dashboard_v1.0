...
function setMasPolicy(type, e1){
	...
	e1["label_text"] = e1["label_text"];
	e1["si_group"] = e1["si_group"];
	}
	var selectedNew = "";
	...
	$("#" + id).append("<option data-type='" + type + "' data-policyNo='" + e1["policy_detail_id"] + "' data-customer = '" + e1.premium + "'  data-id = '" + e1.PremiumServiceTax + "' family_child= '" + e1.child + "' family_adult= '" + e1.adult + "' value =" + e1.sum_insured + " "+selectedNew+"' data-si = '" + e1.si_group+"' >" + e1.label_text + "</option>");
	}
	setFamilyConstruct();
    }
...
function setFamilyConstruct(value) {
	...
	if(value == '' || value == undefined){
		...
		$("#patFamilyConstruct").html('<option value="">Select</option>');
		...
		$("#patFamilyConstruct").empty();
		$("#patFamilyConstruct").html('<option value="">Select</option>');
		$("#patFamilyConstruct").append("<option value='1A' "+cls+">1A</option><option value='2A' "+cls1+">2A</option>");
		$("#patFamilyConstructDiv").show();
	}
...
function addDependentForm(tbody, elem) {
    ...
    elem.data.forEach(function (e) {
		...
		//alert(e.relationship.toUpperCase());
	    var str = "<tr class = "+ucwords(e.relationship)+">";
 ...
 //var selectChange = $("#" + tbody).closest("form");
    ...
    $("#premium").val(premium_ghi+premium_gpa+premium_vtl);
	get_Table_data_show_hide();
	auto_renewal_flag();
}


function getSelectedLabel(getRelationProposer,sumInsures,patFamilyConstruct){

	// alert(sumInsures);
	// alert(patFamilyConstruct);
	if(sumInsures<=1000000&&patFamilyConstruct=='2A'){
		$('.annual_label').html('Annual Income of Self');
		$('.occu_label').html('Occupation of Self');
		// alert('one');
	}else if(sumInsures>1000000&&patFamilyConstruct=='2A'){
		$('.annual_label').html('Annual Income of Self<span style="color:#FF0000">*</span>');
		$('.occu_label').html('Occupation of Self<span style="color:#FF0000">*</span>');
		// alert('two');	
	}else if(sumInsures>1000000&&patFamilyConstruct=='1A'){
		$('.annual_label').html('Annual Income of '+(getRelationProposer==0?'Self':'Spouse')+'<span style="color:#FF0000">*</span>');
		$('.occu_label').html('Occupation of '+(getRelationProposer==0?'Self':'Spouse')+'<span style="color:#FF0000">*</span>');
		// alert('three');
	}else if(sumInsures>1000000){
		$('.annual_label').html('Annual Income of '+(getRelationProposer==''?'Self':'Spouse')+'<span style="color:#FF0000">*</span>');
		$('.occu_label').html('Occupation of '+(getRelationProposer==''?'Self':'Spouse')+'<span style="color:#FF0000">*</span>');
		// alert('four');
	}else{
		$('.annual_label').html('Annual of '+(getRelationProposer==0?'Self':'Spouse'));
		$('.occu_label').html('Occupation of '+(getRelationProposer==0?'Self':'Spouse'));
		// alert('five');
	}
...
function changes(e) {
	...
	var getRelationProposer=$(e).val();
	var sumInsures = $("#sum_insures :selected").val();
	var patFamilyConstruct=$("#patFamilyConstruct").val();
	getSelectedLabel(getRelationProposer,sumInsures,patFamilyConstruct)
...
function get_all_policy_data() {
	...
	$("#occupation").val('');
	$("#annual_income").removeAttr('readonly', 'readonly');
	$("#annual_income").removeAttr('disabled', 'disabled');
		}
		else
		{			
			annual_income_hide_show();
			...
			$("#annual_income").attr('disabled', 'disabled');
		}
	}
	}
	debugger;
...
$(document).ready(function () {
    ...
    $("#getMaxPremiumAgeForm").validate({
        ...
        submitHandler: function (form) {
            ...
            $.post("/get_premium_member_age", { "policyNo": policyNo, "sum_insures": sum_insures, "familyConstruct": familyConstruct, "maxPremiumAge": maxPremiumAge }, function (e) {
                e = JSON.parse(e);
	...
	function getPremiumByMemberAge(id) {
		...
		{
			if(edit == 0 || edit == "")
			//$("#getMaxPremiumAgeModal").modal(); commented -
			var get_family_const = $("#family_const_id").val();
			selectChange = $("#"+get_family_const).closest("form");
            ...
            var dob = $("#dob").val();
            var maxPremiumAge = 0;
            $.post("/get_premium_member_age", { "policyNo": policyNo,"dob":dob ,"sum_insures": sum_insures, "familyConstruct": familyConstruct, "maxPremiumAge": maxPremiumAge }, function (e) {
                 e = JSON.parse(e);
                selectChange.find("input[name='premium']").val("");
                if (e.status) {
					if(get_family_const == 'patFamilyConstruct')
    ...
    $("#patFamilyConstruct").on("change", function () {
		...
		var getRelationProposer=$("#family_members_id1 :selected").val();
		var sumInsures = $("#sum_insures :selected").val();
		var patFamilyConstruct=$("#patFamilyConstruct").val();
		getSelectedLabel(getRelationProposer,sumInsures,patFamilyConstruct)
        ...
        else if (sumInsuresType == "memberAge") {
            getPremiumByMemberAge("patFamilyConstruct");
		 //	getRelation();
        }
    });
	...
	$("#premiumBif").on("click", function () {
    	...
    	var str = '';
		var si = $("#sum_insures option:selected").attr("data-si");
		var si_trim = si.trim();

		var si_split = si_trim.split(',');
		...
		str += "<div class='mr-3'><span style='color:#da8085;'> Sum Insured </span>" + "<br/><span style='color:#000;'>" + si_split[0] + "</span></div>";
		str += "<div><span style='color:#da8085;'> Premium" + "</span><br/><span style='color:#000;'>" + parseFloat($("#premiumgci").val()).toFixed(2) + "</span></div></div>";
		str += "<div style='display: flex; flex-direction:row; justify-content:space-between; padding-top:10px;'><div><span style='color:#da8085;'> Name:  " + "<br/></span><span style='color:#000;'>Group Critical illness</span></div>";//new CR changes 271021
		str += "<div class='mr-3'><span style='color:#da8085;'> Sum Insured </span>" + "<br/><span style='color:#000;'>" + si_split[1] + "</span></div>";
		str += "<div><span style='color:#da8085;'> Premium" + "</span><br/><span style='color:#000;'>" + parseFloat($("#premiumgpa").val()).toFixed(2) + "</span></div></div>";
...
function saveDocumentsAfterValidate() {
	...
	swal({
	title: "Success",
	text: "Link Sent to customer for OTP Verification",//"Proposal Submitted Successfully",
	type: "warning",
	showCancelButton: false,
	confirmButtonText: "Ok!",
	closeOnConfirm: true,
...
function confirm() {
	...
	{	
		if(tmpew_status == 1 && is_non_integrated_single_journey == 0){ //single link journey - 28-9-21
			ajaxindicatorstop();
			swal("Alert", "IMD code is not created for this branch. Please share IMD code creation request with ABHI team to process further");
			return false;
		}
...
function apply_button() {
	...
	{sum_annual = annual_income_logic();if(sum_annual == false){return;} }else{swal("Alert", "please enter annual Income");return;}}else{annual_income_hide_show();}
