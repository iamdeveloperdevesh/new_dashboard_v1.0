...
$("#auto_renewal_flag").click(function(){
	if($(this).prop("checked") == true){
		...
		if(total_amount != ''){
			$("#si_amount").val(total_amount*1.5+ " (up to 150% of Initial Premium Amount)");
		}else if($("#premium").val() != ''){
			$("#si_amount").val($("#premium").val()*1.5+ " (up to 150% of Initial Premium Amount)"); 
		}	
	var acc_no = $('#bankAcNo').val();
		var trailingCharsIntactCount = 4;
		var si_account_number = new Array(acc_no.length - trailingCharsIntactCount + 1).join('x')
	       + acc_no.slice(-trailingCharsIntactCount);
