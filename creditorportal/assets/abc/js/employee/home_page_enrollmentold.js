...
function changes(e) {
    ...
    // var $family_date_birth = $(".family_date_birth");
    // $family_date_birth.val("");
    // $.ajax({
    // url: "/employee/get_min_max_age",
    // type: "POST",
    // data: {
    // rel_id: $("#family_members_id")
    // .find("option:selected")
    // .val(),
    // policy_id: $("#policy_no").val() //$("#policy_no option:selected").val()
    // },
    // dataType: "json",
    // success: function (response) {
    // if (
    // $("#family_members_id").val() == 2 ||
    // $("#family_members_id").val() == 3
    // ) {
    ...
    // "option",
    // "yearRange",
    // "-" + response.max_age + ":-" + response.min_age
    // );
    // $family_date_birth.datepicker(
    // "option",
    // "minDate",
                        ...
                        var dataOpt = $("#family_members_id1 :selected").attr("data-opt");
                           ...
                           if(gen == 'Spouse/Partner')
                           {
                                ...
                                if($("#gender1").val() == 'Male' )
                                {
                                    ...
                                    $('#family_gender1').html("<option value=''>Select</option><option value='Female'>Female</option>");
            
                                }
                                else{
                                    $('#family_gender1').html("<option value=''>Select</option><option value='Male'>Male</option>");
                                }
                           }
                           else
                           {

                            $('#family_gender1').html("<option value=''>Select</option><option value='"+dataOpt+"'>"+dataOpt+"</option>");
...
$(document).ready(function () {
    ...
    $(document).on("click", "#modal-submit", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.length != 0) {
                    $("#nominee_fname").val(response.family_firstname);
                    $("#nominee_lname").val(response.family_lastname);
    ...
    $("#emp_form_old").validate({
        ...
        submitHandler: function (form) {
            ...
            var all_data = $("#emp_form_old").serialize();
            $.ajax({
                type: "POST",
...
function editPopulate(e1) {
    ...
    $.post("/get_edit_member", {
        ...
        family_gender.val(e.policy_mem_gender);
        ...
        success: function (response) {
            ...
            if (family_detail.length != 0) {
                ...
                if (family_detail[0].fr_id == "2" || family_detail[0].fr_id == "3") {
                    ...
                    $("#nominee_fname").val(family_detail[0].family_firstname);
                    $("#nominee_lname").val(family_detail[0].family_lastname);
    ...
    if (!status) {
        ...
        $('#policy_nominee_form_data .form_row_data1').each(function (key, value) {
            ...
            if ($(this).find('.nominee_contact').val() == '') {
                ($(this).find('.nominee_contact')).after("<span style='color:red;'>Contact No is required</span>");
                //location.reload();
                status = false;
            }
...
function confirm() {
    // debugger;
    //get_employee_lead_id();return false;
    
    ajaxindicatorstart();
    var mydeclare = $("#mydatas input[type='radio']:checked").val();

    if (mydeclare == 'Yes') {
        swal(" Basis information provided this proposal cannot be processed online");
        return false;
