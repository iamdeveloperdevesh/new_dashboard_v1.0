...
all_hospitals: true
    ...
    $('#cities').on('change', function() {
        $.ajax({
            ...
            type: "POST",
            data: {
                "state_names": $("#state_names option:selected").text(),
    ...
    $('#search_hopsitals_search').on('click', function() {
        $.ajax({
            url: "/employee/get_hospitals_by_name",
            type: "POST",
            data: {
	...
	debugger;
       ...
       // var address = "new york";
        ...
        var output_from_api = "https://maps.googleapis.com/maps/api/geocode/json?address="+encodeURIComponent(hospital_address)+"&key=AIzaSyAo0EbmDskwdF5JMjFPPijp8w5DqHrrVfY"
...
/*$(document).ready(function() {
var tpa_id = $("#policy_no option:selected").text();
    setTimeout(function() {
        $("#policy_no").change();
    }, 2000);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (i = 0; i < response.length; i++) {
               ...
               // if (date > current_date) {
                    if (response[i].policy_sub_type_id == 1) {
                        $('#policy_no').append('<option selected value="' + response[i].policy_no + '">' + response[i].TPA_id + '</option>');
                    }
               // }
            }
        }
    ...
    $("#policy_no").change(function() {
		...
		$.ajax({
            ...
            data: {
                policy_no: value,
				"tpa_id" : $("#policy_no option:selected").text(),
				all_hospitals: true
            },
            async: false,
            success: function(res) {
