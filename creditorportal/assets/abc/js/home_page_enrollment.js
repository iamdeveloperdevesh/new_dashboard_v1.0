...
$(document).ready(function () {
	//$('#auto_renewal_flag').prop( "checked", true);
...
function addDependentForm(tbody, elem) {
    ...
    elem.data.forEach(function (e) {
        ...
        str += "<td>" + e.policy_member_first_name + "</td>";
        str += "<td>" + e.policy_member_last_name + "</td>";
...
$('#submit_nominee').on("click", function () {
	...
	if (status) {
		...
		$.ajax({
			type: "POST",
			url: "/employee/home/validate_upload_data",
			data: data,
			processData: false,
			contentType: false,
			cache: false,
...
function validate_proposal(){
	...
	$.ajax({
		url: "/employee/home/validate_proposal",
		type: "POST",
		async: false,
		dataType: "json",
		data: {
