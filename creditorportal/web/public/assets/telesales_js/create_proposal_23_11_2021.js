...
function ajaxindicatorstart(text) {
  ...
  function ajaxindicatorstop() {
   ...
   $(window).on('load', function() {
	...
	//space removed changes done by sonal
$('body').on("blur", "#avName,#agent_name,#tl_name,#am_name,#om_name,#firstname,#lastname,.first_name1,.last_name1,#nominee_fname,#nominee_lname", function () {
...
/*$(document).ajaxSend(function(event, request, settings) {
    ...
    $(document).ready(function () 
    {
    	...
    	$("body").delegate("#spouse_date_birth", "focusin", function () {
	...
	$(document).on("click",".xlmodalvalidate",function(){
		if($('input[name="policy_selection"]:checked').val() == undefined){
			...
			if(policy_detail_id == TELE_HEALTHPROINFINITY_GHI_GPA){
				...
				$.ajax({
					url: "tele_gpa_policy_purchased_for_self",
					type: "POST",
					async: false,
					dataType: "json",
					success: function (response) {
		...
		$.validator.addMethod("notEqual1", function(value, element, param) {
			if(value != 2 && value != 3){
				//alert(value);
			  return this.optional(element) || $(param).not(element).get().every(function(item) {
				  return $(item).val() != value;
		...
		$.validator.addClassRules("mem_mob_no1", {
		  //notEqual1: ".mem_mob_no1",
		  valid_mobile : "true",
		});

		$.validator.addClassRules("mem_email_id1", {
		  validateEmail : "true"
		});
				 ...
				 $.ajax({
						...
						{////debugger;
								...
								if(data!='')
								{
									...
									if(product_id == 'T01' || product_id == 'T03' || product_id == 'R06'){ //ankita ghd changes
...
//dedupe
	...
	function getPremiumNew(){
		// alert($('select[name=sum_insure]').val());
		// alert($('select[name=sum_insure] :selected').attr("data-policyno"));
		if($("#plan_name").val() != "T03"){
			var policy_detail_id =  $('select[name=sum_insure] :selected').attr("data-policyno");
		}else{
			// var policy_detail_id =  $('select[name=policy_selection] :selected').val();
			var policy_detail_id =$("input[name='policy_selection']:checked").val()
		}
		// alert(policy_detail_id);
		// return;
		$.post("/tele_get_premium", { "sum_insured":  $('select[name=sum_insure]').val(), "policy_detail_id": policy_detail_id, "family_construct":  $('select[name=familyConstruct]').val(),"gci_optional":  $('input[name=GCI_optional]:checked').val(),"product_id" : $('#plan_name').val() }, function (e)
								{
									//////debugger;
									$("#premiumModalHidden").val(e);
									e = JSON.parse(e);
	...
	submitHandler: function (form) {
		...
		if($("#plan_name").val() == 'T03'){
			var finalPremium = $("#premium").val();
		}else{
			var finalPremium = premium_pat;
		}
		var form = $("#patForm").serialize() + "&plan_name="+$('#plan_name').val()+ "&GCI_optional="+$("input[name='GCI_optional']:checked").val()+"&premium=" + finalPremium + "&policyNo=" + $("#sum_insures :selected").attr("data-policyno") + "&familyDataType=" + familyDataType + "&declare=" + JSON.stringify(TableData2)+ "&chronic=" + JSON.stringify(TableData3);
		...
		// Added by Akash on 2021-08-27
		var old_premium = $('#premium').val();
		...
		// Added by Akash on 2021-08-27
			...
			new_changed_premium = data.data[0].new_premium;
			 ...
			 if($("#plan_name").val() == "T03"){
				...
				$.ajax({
					url:get_all_data(),
					success:function(){
						new_changed_premium = $("#premium").val();
				 }
				 });

				 if(old_premium!=new_changed_premium){
					is_premium_got_changed = 1;
				}else{
					is_premium_got_changed = 0;
				}
		...
		// Added by Akash on 2021-08-27
				...
				if(is_premium_got_changed == 1){
					var msg1 = "Premium has been changed as per your inputs to "+new_changed_premium;
					var status1 = "warning";
				}else{
   ...
   $(document).on("click", "#modal-submit", function () 
	...
	$("select[name='familyConstruct']").on("change", function () 
	{
		...
		//alert(plan_name);
		//upendra - 29-06-2021
		if(plan_name == 'T03' || plan_name == 'T01'){
			
			//if($('.commonCls').length == 0 && $('#sum_insures').val() != '' && !$('#patFamilyConstruct').is('[disabled=disabled]')) {if($('.commonCls').length == 0 && $('#sum_insures').val() != '' && !$('#patFamilyConstruct').is('[disabled=disabled]')) {
				if(show_popup_new == 0 && $('#sum_insures').val() != '' && (!$('#patFamilyConstruct').is('[disabled=disabled]') && $('#patFamilyConstruct').val() !='')) {
	    		...
	    		$.ajax({
					...
					success: function (response) {
						if(response.spouse_dob){
							/*var dateAr = response.spouse_dob.split('-');
							var spouse_dob = dateAr[2] + '-' + dateAr[1] + '-' + dateAr[0];*/
							
							$("#spouse_date_birth").val(response.spouse_dob);
						}
						if(response.kid1_dob){
							/*var dateAr = response.kid1_dob.split('-');
							var kid1_dob = dateAr[2] + '-' + dateAr[1] + '-' + dateAr[0];*/
							$("#kid1_date_birth").val(response.kid1_dob);
						}
						if(response.kid2_dob){
							/*var dateAr = response.kid2_dob.split('-');
							var kid2_dob = dateAr[2] + '-' + dateAr[1] + '-' + dateAr[0];*/
				...
				//upendra - 29-06-2021
				if(plan_name == 'T01'){
					var mymodal = $('#helathproXlFamilyModal');
   				    mymodal.find('.header-title').text('Tele - Health Pro');
				}
	...
	{	
		...
		if($("#family_members_id"+i).val() == 0){			
				...
				if($("#family_members_id"+i).val() != ''){
					swal({
						...
						function () {
							...
							$('.cust_div_col').removeClass("collapsed");
							$('#accordion223').removeClass("collapsing");
							$('#accordion223').addClass("collapse show");
							setTimeout(function() { $('#firstname').focus(); }, 2000);
							$('#self_edit_clicked').val(1);
							$("#edit_btn_custsection").trigger("click");
						   	return false;
			...
			if(proposal_created == 1){
				...
				$("#mem_email_id"+i).prop('disabled',false);
				$("#mem_mob_no"+i).prop('disabled',false);
			}else{
		...
		if(mem_id != "undefined"){
			...
			if(($("#patFamilyConstruct").val() != '' || $("#patFamilyConstruct").val() != undefined) && ($("#sum_insures").val() != '' || $("#sum_insures").val() != undefined)){
							...
							if(divCount != count ){
								...
								$('#family_members_id'+i).val('');
								$('#family_salutation'+i).val('');
								$('#family_gender'+i).val('');
								$('#first_name'+i).val('');
								$('#last_name'+i).val('');
								$('#last_name'+i).val('');
								$('#family_date_birth'+i).datepicker('setDate', null);
								$('#age'+i).val('');
								$('#mem_email_id'+i).val('');
								$('#mem_mob_no'+i).val('');
							}
					...
					success: function (response) 
					{
					    ...
					    else 
						{
							...
							for (var i = 0; i < response.data.length; i++) 
							{
								...
								{
									membersCovered.push(response.data[i]['fr_id']);
								   //$("#sum_insures").attr("disabled", "disabled");
								  
								   $("#sum_insures").val(response.data[i]['policy_mem_sum_insured']);
							       //$('#premium').val(response.data[i]['policy_mem_sum_premium']+".00");
							       $('#premium').val(response.data[i]['policy_mem_sum_premium']);
								   
							    //    $('#premium').val('testing12');
								    //$("#patFamilyConstruct").empty();
								 ...
								 // $("#patFamilyConstruct").append('<option value = "'+response.data[i]['familyConstruct']+'" data-premium = "'+response.data[i]['policy_mem_sum_premium']+'">'+response.data[i]['familyConstruct']+'</option>');
									if(premAmt == 0){
										// console.log(response.data[i]['policy_mem_sum_premium']);
										premAmt = parseFloat(response.data[i]['policy_mem_sum_premium']);
									}
...
////debugger;
						   ...
						   if($('#plan_name').val() == 'T03'){
						    	// $('#premium').val(premAmt+".00");
						    	if(cntelem == $('.commonCls').length){
						    		$('#premium').val(premAmt);
						    	}
						...
						//upendra - new health pro - rater changes
						if($('#plan_name').val() == 'T01'){
							getPremiumNew();
						}
					}
				...
				salutation: 
					...
					{////debugger;
						...
						success: function (response) 
						{
							...
							var totalfamily_length=$('.family_members_id1').length;
							var get_member_check='';
							for(var i=0;i<=totalfamily_length-1;i++){
								var member_check=$("#family_members_id"+i).val();
								if(member_check==0){
									get_member_check=member_check;
								}
							}

							// alert(get_member_check);
							var res = response;
							...
							if (res.status == true) 
							{
								...
								swal({
									...
									var empSalutation = $("#salutation").val();
											...
											$('#mem_email_id'+get_member_check).val($('#email').val());
							}
						}
						});
					}
			...
			rules:  {
				...
				nominee_fname: 
					...
					submitHandler: function (form) 
					{
						...
						var all_data = $("#nominee_data").serialize()+"&nominee_relation="+$("#nominee_relation").val()+"&nominee_dob="+$('#nomineedob').val()+"&nominee_salutation="+$("#nominee_salutation").val()+"&nominee_contact="+$('#nominee_contact').val()+"&nominee_email="+$("#nominee_email").val();
						$.ajax({
...
/* end */			
			...
			var all_data = $("#payment_details").serialize();
						...
						success: function (response) 
						{
							$("#payment_details :input").prop("readonly", true);
							debugger;
							...
							$.each(response.data, function(index, item) {
								str+= '<tr><td>'+item.Dispositions+'</td><td> '+(item["Sub-dispositions"])+'</td><td>'+item.date+'</td><td>'+((item.agent_name)?item.agent_name:'AV')+'</td><td>'+((item.remarks) ? item.remarks : '')+'</td></tr>';
						});
		...
		if(check_ghd == false)
		{
			...
			{
				ajaxindicatorstop();
				swal("Alert","As the Customer Declaration Question is No, the proposal cannot be processed", "warning");
				$("#myModal1").modal("hide");
				$("#sms_body").html('');
				return false;
...
var remarks = '';
		...
		//updated by upendra on 09-04-2021
		if($("#plan_name").val() == 'T03' || $("#plan_name").val() == 'T01' || $("#plan_name").val() == 'R06'){//ankita ghd changes
			remarks = '';
	...
	$("#nominee_contact").keyup(function (e) 
	{
		var $th = $(this);
		if (
		  e.keyCode != 46 &&
...
/*
	$("#agent_id").bind("focusout", function() 
	{
       var agents =  $(this).val();
	      if(agents.length >= 1) 
		  {
...
function agent_cust_prefilled(response)
{
	...
	if((product_id == "T03" || product_id == "T01" || product_id == "R06") && (emp.new_remarks)){ //ankita ghd changes
...
var is_maker_checker = $("#hidden_is_maker_checker").val();
var is_maker_checker_edit=$('#checker_edit').val();

if(is_maker_checker == "yes"||is_maker_checker_edit=="yes"){
	$("#disposition").val('');
	$("#disposition").attr('disabled', false);
	$("#sub_isposition").val('');
	...
	var base_agent_id =  $("#hidden_base_caller_id").val();
	var base_agent_lob =  $("#hidden_base_caller_lob").val();
	var base_agent_center =  $("#hidden_base_caller_center").val();
	var base_agent_vendor =  $("#hidden_base_caller_vendor").val();
	var base_agent_name =  $("#hidden_base_caller_name").val();

	var base_tl_id =  $("#hidden_base_caller_tl_id").val();
	var base_tl_name =  $("#hidden_base_caller_tl_name").val();
	var base_imd =  $("#hidden_base_caller_imd").val();

	$("#agent_id").val(base_agent_id);
	$("#agent_id").css( 'pointer-events', 'none' );
	
	$("#axis_location").val(base_agent_center);
	$("#axis_lob").val(base_agent_lob);
	$("#agent_name").val(base_agent_name);

	$("#tl_id").val(base_tl_id);
	$("#imd_code").val(base_imd);
	$("#tl_name").val(base_tl_name);

	$("#axis_location").trigger("change");
	$("#axis_vendor").val(base_agent_vendor);
	

	$("#plan_name > option").each(function() {
		var text1 = $(this).text();
		if(text1 == "Group Activ Health-Tele"){
			$(this).text(text1.replace('Group Activ Health-Tele', 'Group Activ Health')); 
		}
		if(text1 == 'Tele - Health Pro Infinity'){
			$(this).text(text1.replace('Tele - Health Pro Infinity', 'Health Pro Infinity')); 
		}
		
	});	
	
	if(is_maker_checker_edit == "yes"){
		$('input[name="dob"]').addClass("dis-col");
		
		$.ajax({
			url:proposal_create_check(),
			success:function(){
				$('.family_date_birth').addClass("dis-col");
				$("#emp_data :input").prop("readonly", true); 
...
$(document).on('click','#pre_send',function(){
	...
	var remark_lead=$('#remark_lead').val();
	var remark_emp_id=$('#remark_emp_id').val();
	var remark_agent_name=$('#remark_agent_name').val();
	var pddisposition=$('#disposition').val();
	
	$.ajax({
		url:'/save_maker_checker_remark',
		type:'post',
		dataType: 'json',
		data:{remark:remark,remark_lead:remark_lead,remark_emp_id:remark_emp_id,remark_agent_name:remark_agent_name,pddisposition:pddisposition},
		success:function(response){
			if(response.status==1){
				...
				allowEscapeKey: false
					...
					window.location.href='maker_checker_view';
					// location.replace('/maker_checker_view');
				});			
