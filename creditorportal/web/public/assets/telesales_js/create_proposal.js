...
//         if (spc)
//         {
//             var rp = this.value.replace(/[^a-zA-Z0-9, ]/g, "");
//             $(this).val(rp);
//         }
...
$(document).ready(function ()
{
	...
	$(document).on("click","#edit_btn_custsection",function(){
		...
		if(proposal_created == 1){
			...
			$('#comAdd2').attr('readonly',false);
			$('#comAdd3').attr('readonly',false);
			$('#pin_code').attr('readonly',false);
			$('#mobile_no2').attr('disabled',false);
...
//dedupe
	...
	if($('select[name=sum_insure]').val() != '' && $('select[name=sum_insure] :selected').attr("data-policyno") != '' && $('select[name=familyConstruct]').val() != '' && $('#plan_name').val() != ''){
		$.post("/tele_get_premium", { "sum_insured":  $('select[name=sum_insure]').val(), "policy_detail_id": $('select[name=sum_insure] :selected').attr("data-policyno"),"deductable":$('#hidden_deductable').val(),"family_construct":  $('select[name=familyConstruct]').val(),"gci_optional":  $('input[name=GCI_optional]:checked').val(),"product_id" : $('#plan_name').val() }, function (e)
		{
			////debugger;
...
$("select[name='familyConstruct']").on("change", function ()
{
	...
	if(plan_name == 'T03' || plan_name == 'T01'){
		console.log(show_popup_new);
		console.log($('#sum_insures').val());
		console.log($('#patFamilyConstruct').is('[disabled=disabled]'));
		console.log($('#patFamilyConstruct').val());
		...
		if(show_popup_new == 0 && $('#sum_insures').val() != '' && (!$('#patFamilyConstruct').is('[disabled=disabled]') && $('#patFamilyConstruct').val() !='')) {
			...
			if(plan_name == 'T01'){
				...
				// 03-02-2022
				mymodal.find('.header-title').text('Health Pro');
			}else if(plan_name == 'T03'){
				var mymodal = $('#helathproXlFamilyModal');
				mymodal.find('.header-title').text('Health Pro Infinity');
			}
...
function hideEditButtons() {
	var hidden_editstatus=atob($("#hidden_editstatus").val());
	console.log(hidden_editstatus);
	if(hidden_editstatus == '0'){
		$('.editBtn').hide();
	}
}
