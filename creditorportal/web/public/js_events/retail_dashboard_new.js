...
$( document ).ready(function() {
	var flashMessage = $("#flash_message").val();
  	if(flashMessage != '' && flashMessage != undefined){
    	// swal(flashMessage);
    	sad_icon("Please submit the quote first !!", false);
    	// swal("Alert","Please submit the quote first!!","warning");
  	}
	// return false;
...
function showToolTipModal(elem){
	...
	var policyImage = {
		'5 to 25 Lacs':{
			'1':'/public/assets_new/images/hp1-new-2021.png',
			'2':'/public/assets_new/images/hp2.png',
			'3':'/public/assets_new/images/hp3.png'
		},
		'50 to 100 Lacs':{
			'1':'/public/assets_new/images/hpi1-new-2021.png',
			'2':'/public/assets_new/images/hpi2-new.png',
		}
	};

	if($(elem).attr('tool-type') == 'product'){
		// $('#tooltip-header').html('Group Active Health');
		$('#tooltip-header').html('Group Activ Health/Group Activ Secure');
        ...
        var footer_content=JSON.parse(selectedCombo[0].tooltip_content);
        ...
        $.each(selectedCombo[0].policies, function(key, policy){
        	...
        	if(deductible == '0'){
        		...
        		imageHtml = "<img src='/public/assets_new/images/hpi3-new.png'>";
        	}
        });

        imageHtml += "<p style='text-align: left;font-weight: 800;'>"+footer_content.tooltip_bottom_note+"</p>";
		...
		$('#tooltip-footer').html('');
		// if(footer_content.tooltip_bottom_note != undefined){
		// 	$('#tooltip-footer').html('<b>  '+footer_content.tooltip_bottom_note+'</b>');
		// }
	...
	if($(elem).attr('name') == 'adults'){
		if($(elem).is(':checked')){
			...
			if($('[name="adults"]:checked').length == 0){
				...
				sad_icon("Atleast one adult is required!!",false);
		...
		if(value != 0){
			for (var i = 0; i < value; i++) {
				...
				$.each(childFamilyConstructs, function(key, childConstruct){
					...
					// var modifiedFrName = childConstruct.fr_name;
...
function updateRelationshipId(selectedFrId,type,removeFlag){
...
function changeProduct(elem){
	...
	if(checkedAdults.length == 0){
		...
		sad_icon("Please select atleast 1 adult !!",false);
		return false;
	}
...
function occupationLogicBoxShowHide(){
	...
	fieldsRequired = checkOccAiMandatory();
	if(fieldsRequired){
		asterikHtml = "<sup class='star_sup'><img src='/public/assets_new/images/important.png'></sup>";
		mandatoryFlag = true;
	}
	...
	if(employeeToProductOnLoad && employeeToProductOnLoad.sum_insured_amt){
		...
		if(employeeToProductOnLoad.sum_insured_amt > 1000000){
			if(employeeToProductOnLoad.check_occupation_ai == 1){
				occupationHtml = "<div class='col-md-4 form-group mb-2 occupation-logic-box'><label class='lb-1'>"+occupationLabel+""+asterikHtml+"</label><select class='form-control' name='occupation' mandatory="+mandatoryFlag+" id='occupation_box' type='text' placeholder='Select Occupation'><option value=''>Select Occupation</option>"+occupationListHtml+"</select></div><div class='col-md-4 form-group mb-2 occupation-logic-box'><label class='lb-1'>"+annualIncomeLabel+""+asterikHtml+"</label><input class='form-control' mandatory="+mandatoryFlag+" type ='number' maxLength='10' onkeyup='onlyNumbers(this)' name='annual_income' value='"+annualIncomeSelected+"' id='annual_income_box'></div>";
				$(".occupation-logic-box").remove();
				$("#occupation_sum_box").append(occupationHtml);
			} else {
				$(".occupation-logic-box").remove();
			...
			if(employeeToProductOnLoad.check_occupation_ai == 1){
				occupationHtml = "<div class='col-md-4 form-group mb-2 occupation-logic-box'><label class='lb-1'>"+occupationLabel+"</label><select class='form-control' mandatory="+mandatoryFlag+" name='occupation' id='occupation_box' type='text' placeholder='Select Occupation'><option value=''>Select Occupation</option>"+occupationListHtml+"</select></div>";
				$(".occupation-logic-box").remove();
				$("#occupation_sum_box").append(occupationHtml);
			} else {
				$(".occupation-logic-box").remove();
...
function checkOccAiMandatory(){
	var occupationListMandate = false;
	$.ajax({
		url: "/check_occupation_list_mandate",
		type: "POST",		
		dataType: "json",
		async: false,
		success: function (response){
			if(response.status){
				occupationListMandate = true;
			} else {
				occupationListMandate = false;
			}
		}
	});
	return occupationListMandate;
}

function getOccupationOptions(productId){
    	...
    	$.ajax({
			...
			success: function (response) {
				if(response.status == 'success'){
					...
					sad_icon(response.message, false);
				}
			}
    	});      
    }
...
function submitCombination(elem){
	...
	if(checkedComboElement.length == 0){
		...
		sad_icon("Please select combination!!", false);
	}
...
function validateConfirmation(){
	...
	$.ajax({
	   ...
	   //              closeOnConfirm: true
	   //          });
	            sad_icon(response.message[0],false);
