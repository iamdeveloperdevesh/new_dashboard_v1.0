		...
		if(!empty($userListing['Data']['query_result']) && count($userListing['Data']['query_result']) > 0)
		{
			for($i=0;$i<sizeof($userListing['Data']['query_result']);$i++)
			{
				...
				//if(in_array('UserEdit',$this->RolePermission)){
					$actionCol .='<a href="assignmentdeclaration/addEdit?text='.rtrim(strtr(base64_encode("id=".$userListing['Data']['query_result'][$i]['assignment_declaration_id'] ), '+/', '-_'), '=').'" title="Edit"><i class="fa fa-edit"></i></a>';
				//}
				//if(in_array('UserDelete',$this->RolePermission)){
					if($userListing['Data']['query_result'][$i]['is_active'] == 1){
						$actionCol .='&nbsp;&nbsp;<a href="javascript:void(0);" onclick="deleteData(\''.$userListing['Data']['query_result'][$i]['assignment_declaration_id'] .'\');" title="Delete"><i class="fa fa-trash"></i></a>';
					}
				//}
