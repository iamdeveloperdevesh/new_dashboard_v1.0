...
class Apimodel extends CI_Model
{
	...
	{
		$this->db->select('mp.*,master_plan.creditor_id,master_plan.gender,master_plan.coi_download,master_plan.is_single_coi,master_plan.pan_mandatory,master_plan.nominee_mandatory,master_plan.self_mandatory,master_plan.plan_name,master_plan.coi_type,master_plan.policy_type_id,mpst.policy_sub_type_name,mc.creaditor_name as creditor_name,payment_first,payment_page');
	...
	function getProductDetailsAll_diff($id, $sort_by = '', $order = '', $policy_id_arr = [])
	{
		...
		$this->db->join('master_policy_premium_basis_mapping as mppb', 'mppb.master_policy_id = mp.policy_id');
		$this->db->join('master_policy_si_type_mapping as mpsi', 'mpsi.master_policy_id = mp.policy_id');
		//$this->db->join('master_policy_mandatory_if_not_selected_rules as mpns', 'mpns.master_policy_id = mp.policy_id');
