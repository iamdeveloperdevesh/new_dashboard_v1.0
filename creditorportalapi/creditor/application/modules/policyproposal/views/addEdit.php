...
<div class="page-body">
	...
	<?php if($changepaymentmodeonly != 1){ ?>
...
$(".nominee_relation").change(function(){
    ...
    if(val != ""){
        if(val == "Male"){
            $(this).find(".nominee_salutation").val("Mr");
            $(this).find(".nominee_gender").val("Male");
            $(this).find(".nominee_female_gen").css('display','none');
            $(this).find(".nominee_male_gen").css('display','block');
        }else{
            $(this).find(".nominee_gender").val("Female");
            $(this).find(".nominee_salutation").val("");
            $(this).find(".nominee_salutation").attr("readonly",false);
            $(this).find(".nominee_female_gen").css('display','block');
            $(this).find(".nominee_male_gen").css('display','none');
        }
    }else{
        $(this).find(".nominee_gender").val("");
        $(this).find(".nominee_salutation").val("");
        $(this).find(".nominee_salutation").attr("readonly",false);
        $(this).find(".nominee_female_gen").css('display','block');
        $(this).find(".nominee_male_gen").css('display','block');
...
$(".memberformsubmit").click(function(){
		var form = "#"+$(this).attr('data-form');
		var act = "<?php echo base_url();?>policyproposal/submitmemberForm";
		var maincontainer = $(form).attr("data-parent");
		var formcontainer = $(form).attr("data-sub-parent");
		var error = 0;
		if($(form).find('.family_gender').val() == ""){
			$(form).find('.family_gender_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.first_name').val() == ""){
			$(form).find('.first_name_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.last_name').val() == ""){
			$(form).find('.last_name_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.age').val() == ""){
			$(form).find('.age_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.relation').val() == ""){
			$(form).find('.relation_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.sum_insured').val() == ""){
			$(form).find('.sum_insured_error').html("This field is required");
			error = 1;
		}
		if($(form).find('.adultcount').val() == ""){
			$(form).find('.adultcount_error').html("This field is required");
			error = 1;
		}
